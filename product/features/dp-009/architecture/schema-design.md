# DP-009 Schema Design: Silver Layer Data Dictionary
...
This document provides the complete DDL for extending the `data_dictionary` schema to include Silver layer metadata. The implementation follows decisions in:
...
### Target Schema: `data_dictionary`

Extends existing Bronze dictionary schema established in dp-002.

### New Tables

| Table | Purpose | PK |
|-------|---------|-----|
| `silver_tables` | Silver table metadata | `table_name` |
| `silver_columns` | Column definitions | `id` (surrogate) |
| `silver_lineage` | Bronze-to-Silver mappings | `id` (surrogate) |
| `silver_dq_rules` | DQ rules per column | `id` (surrogate) |

---

## Complete DDL

### Migration Script: `003_silver_data_dictionary.sql`
...
```sql
-- List all Silver tables with their source streams
SELECT table_name, description, grain, source_streams
FROM data_dictionary.silver_tables;

-- List all columns in a Silver table
SELECT column_name, data_type, unit, description, nullable
FROM data_dictionary.silver_columns
WHERE table_name = 'silver.air_quality_observations'
ORDER BY sort_order;

-- Show complete data dictionary (Bronze + Silver)
...
```sql
-- Where does pm25 column come from?
SELECT source_stream, bronze_path, transformation, bronze_type, bronze_unit
FROM data_dictionary.get_column_lineage('silver.air_quality_observations', 'pm25');
...
WHERE silver_table = 'silver.weather_observations';

-- What Silver tables does air-quality stream feed?
SELECT DISTINCT silver_table
FROM data_dictionary.silver_lineage
WHERE source_stream = 'air-quality';
```
...
SELECT rule_name, rule_params, action, rule_scope
FROM data_dictionary.get_column_dq_rules('silver.air_quality_observations', 'pm25');

-- Show cross-field rules
SELECT silver_table, rule_name, rule_params, action
FROM data_dictionary.silver_dq_rules
WHERE silver_column IS NULL;
```

### Search Queries

```sql
-- Find all columns with unit 'celsius'
SELECT layer, source, column_name, description
FROM data_dictionary.v_column_search
WHERE unit = 'celsius';

-- Find all PM-related columns
SELECT layer, source, column_name, data_type, unit, description
FROM data_dictionary.v_column_search
WHERE column_name LIKE 'pm%' OR description ILIKE '%particulate%';
```
...
Place `003_silver_data_dictionary.sql` in `deploy/pi/init-scripts/`:

```bash
# Copy migration to init-scripts
cp product/features/dp-009/architecture/003_silver_data_dictionary.sql \
   deploy/pi/init-scripts/003_silver_data_dictionary.sql
```
...
For new deployments, init script runs automatically on container start.

### Step 3: Run Sync

After migration, sync will populate the new tables:
...
# Check sync populated data
docker exec pi5-timescaledb psql -U postgres -d ndp -c \
    "SELECT * FROM data_dictionary.v_silver_table_overview;"
```
...
### silver_tables (4 rows)

| table_name | source_streams | grain |
|------------|----------------|-------|
| silver.air_quality_observations | {air-quality} | One row per sensor reading |
| silver.weather_observations | {outdoor-weather,nws-observations} | One row per observation |
| silver.weather_forecasts | {nws-gridpoints-forecast} | One row per forecast period |
| silver.outdoor_air_quality | {outdoor-air-quality} | One row per observation |

### silver_columns (~40-50 rows)

Includes:
- 7 columns from air-quality (pm25, pm10, co2, temperature_c, humidity_pct, tvoc_index, nox_index)
- 10+ columns from weather_observations
- 10+ columns from weather_forecasts
- 10+ columns from outdoor_air_quality
- Plus standard columns per table (observation_time, ndp_id, ingestion_time, dq_flags)

### silver_lineage (~35-40 rows)

One row per (target_column, source_stream) mapping.

### silver_dq_rules (~50+ rows)

- Column-level range_check rules
- Column-level null_check rules
- Cross-field validation rules (silver_column = NULL)

---

## File Location

Save migration script to:

```
/workspaces/neural-data-platform/deploy/pi/init-scripts/003_silver_data_dictionary.sql
```
...
1. [01-create-data-dictionary.sql](../../../../deploy/pi/init-scripts/01-create-data-dictionary.sql) - Bronze dictionary DDL
2. [ADR-009-001](./ADR-009-001-silver-dictionary-tables.md) - Table design decisions
3. [ADR-009-003](./ADR-009-003-sync-mechanism.md) - Sync mechanism design
4. [dp-006 ADR-006-003](../../dp-006/architecture/ADR-006-003-schema-naming-convention.md) - Schema naming
