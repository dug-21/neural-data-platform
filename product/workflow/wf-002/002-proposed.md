...
### 2a: Architecture + Specification (Parallel)

The Design Leader receives the approved SCOPE.md and spawns two specialist agents in parallel:
...
### 2b: Risk Strategy (Sequential — reads Architecture + Specification)

After Architecture and Specification are complete, the Design Leader spawns the risk strategist, which reads both documents plus the product vision criteria to produce a risk strategy grounded in actual design decisions:
...
- Maps each risk to specific testing scenarios, informed by actual component boundaries and interfaces
...
### 2d: Synthesizer Compiles Coordinator Brief
...
3. **Specialist agents, not generalists.** Each document is produced by a specialist — architect, specification writer, risk strategist. The risk strategist runs after architecture and specification, grounding risks in actual design decisions. The coordinator ensures consistency across them.
