...
use gold_fixtures::phase_c::{assert_sql_contains, assert_sql_not_contains, count_sql_occurrences};
...
fn test_zero_objectives_skips_threshold_crossings() {
    ...
    assert_sql_not_contains(&sql, "_crossings", "CG-003: no crossing CTEs at all");
...
fn test_objective_without_unit_produces_valid_sql() {
    ...
    assert_sql_contains(&sql, "safe_voltage", "CG-004: objective id still present");
}
