...
//!
//! ```ignore
//! use ndp_lib::validate::semantic::domain::{validate_domain, validate_domain_semantic};
//!
//! // With known streams
...
///
/// Searches for directories containing config.json files
...
fn discover_streams(streams_dir: &Path) -> HashSet<String> {
    ...
    if let Ok(entries) = std::fs::read_dir(streams_dir) {
        for entry in entries.flatten() {
            ...
            // JSON-only: no YAML config paths
...
/// Extract stream_id from a JSON config file
