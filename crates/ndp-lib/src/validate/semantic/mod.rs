...
/// Shared granularity validation used by both gold.rs and domain.rs
///
/// Pattern: `<number> <unit>` where unit is minute(s), hour(s), or day(s)
pub fn is_valid_granularity(granularity: &str) -> bool {
    let pattern = Regex::new(r"^\d+\s+(minute|hour|day)s?$").unwrap();
