...
use crate::gold::config::{Action, ConfigLoader, DomainConfig, StreamRole};
...
impl ConfigLoader for NullConfigLoader {
    fn load_stream_config(&self, stream_id: &str) -> Result<crate::gold::config::StreamConfig> {
...
impl EventsGenerator {
    ...
    ///
    /// The events TABLE, indexes, retention policy, and unified view are created
    /// by the database init-script (010-events-hypertable.sql) since the events
    /// table is a global cross-domain resource. This generator only produces the
    /// domain-specific components: continuous aggregates, detection procedure,
    /// and detection job.
    ...
    ///
    /// NOTE: The events table is now created by init-script 010-events-hypertable.sql.
    /// This method is retained for the IEventsGenerator trait implementation.
        ...
        DomainConfig {
            ...
            objectives: vec![
                ObjectiveConfig {
                    ...
                    priority: crate::gold::config::Priority::High,
                },
                ObjectiveConfig {
