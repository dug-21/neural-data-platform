...
use crate::gold::generators::column_builder::{ColumnBuilder, DefaultColumnBuilder};
use crate::gold::generators::constants::GOLD_SCHEMA;
use crate::gold::generators::join_builder::{DefaultJoinBuilder, JoinBuilder};
...
impl<L: ConfigLoader> AlignedViewGenerator<L> {
    ...
    fn derive_gold_columns(
        &self,
        stream_config: &crate::gold::config::StreamConfig,
    ) -> Result<Vec<String>> {
...
mod tests {
    ...
    struct MockConfigLoader {
        stream_configs: HashMap<String, crate::gold::config::StreamConfig>,
    }
    ...
    impl MockConfigLoader {
        ...
        fn with_stream(mut self, stream_id: &str, gold_enabled: bool) -> Self {
            let config = crate::gold::config::StreamConfig {
                stream_id: stream_id.to_string(),
                stream_type: None,
                fields: vec![
                    crate::gold::config::FieldConfig {
                        name: "pm25".to_string(),
                ...
                gold_etl: if gold_enabled {
                    Some(crate::gold::config::GoldEtlConfig {
                        enabled: true,
                        aggregates: Some(crate::gold::config::AggregatesConfig {
                            granularities: vec!["1 hour".to_string()],
                            fields: {
                                let mut fields = HashMap::new();
                                fields.insert(
                                    "pm25".to_string(),
                                    crate::gold::config::FieldMetricsConfig {
