...
fn test_no_air_quality_literals_in_events_generator_source() {
    let source = include_str!("../src/generators/events.rs");
...
fn test_state_transitions_no_hardcoded_state_values() {
    let source = include_str!("../src/generators/state_transitions.rs");
    let production_code = strip_tests_and_comments(source);
...
fn test_aligned_view_no_string_based_type_inference() {
    let source = include_str!("../src/generators/aligned_view.rs");
    let production_code = strip_tests_and_comments(source);
...
"#;
    ...
    assert!(!stripped.contains("air-quality"), "test module should be stripped");
    assert!(stripped.contains("production_code"), "production code preserved");
...
"#;
    ...
    let stripped = strip_tests_and_comments(source);
    assert!(stripped.contains("Module documentation"), "doc comments preserved");
}
