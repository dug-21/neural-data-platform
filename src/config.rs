...
fn default_force_shutdown_after_secs() -> u64 {
    60
}
...
fn default_gc_interval_minutes() -> u64 {
    30
}
...
fn default_allowed_origins() -> Vec<String> {
    vec!["https://localhost:3000".to_string()]
}
...
fn default_tcp_keepalive_probes() -> u64 {
    9
}
fn default_log_file_max_size_mb() -> u32 {
    100
}
...
fn default_cloud_backup_provider() -> String {
    "s3".to_string()
}
...
fn default_tls_cert_path() -> String {
    "/etc/ssl/certs/server.crt".to_string()
}
...
impl PlatformConfig {
    ...
    pub fn load(config_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let mut config: PlatformConfig =
            toml::from_str(&config_content).context("Failed to parse configuration TOML")?;
    ...
    fn apply_env_overrides(&mut self) -> Result<()> {
        ...
        if let Ok(ttl) = env::var("REDIS_DEFAULT_TTL_SECONDS") {
            self.redis.default_ttl_seconds =
                ttl.parse().context("Invalid REDIS_DEFAULT_TTL_SECONDS")?;
        }
        if let Ok(timeout) = env::var("REDIS_CONNECTION_TIMEOUT_MS") {
            self.redis.connection_timeout_ms = timeout
                .parse()
                .context("Invalid REDIS_CONNECTION_TIMEOUT_MS")?;
        }
        if let Ok(cluster_mode) = env::var("REDIS_CLUSTER_MODE") {
            self.redis.cluster_mode = cluster_mode.parse().context("Invalid REDIS_CLUSTER_MODE")?;
        }

        // Neural overrides
        if let Ok(memory) = env::var("NEURAL_MEMORY_GB") {
            self.neural.memory_gb = memory.parse().context("Invalid NEURAL_MEMORY_GB")?;
        }
        ...
        if let Ok(port) = env::var("MONITORING_PROMETHEUS_PORT") {
            self.monitoring.prometheus_port =
                Some(port.parse().context("Invalid MONITORING_PROMETHEUS_PORT")?);
        }
        ...
        if let Ok(enable_tls) = env::var("SECURITY_ENABLE_TLS") {
            self.security.enable_tls = enable_tls.parse().context("Invalid SECURITY_ENABLE_TLS")?;
        }
        ...
        if let Ok(enable_file) = env::var("LOGGING_ENABLE_FILE_LOGGING") {
            self.logging.enable_file_logging = enable_file
                .parse()
        ...
        if let Ok(async_logging) = env::var("LOGGING_ASYNC_LOGGING") {
            self.logging.async_logging = async_logging
                .parse()
        ...
        if let Ok(enable_cb) = env::var("CIRCUIT_BREAKER_ENABLE_CIRCUIT_BREAKER") {
            self.circuit_breaker.enable_circuit_breaker = enable_cb
                .parse()
        ...
        if let Ok(recovery_timeout) = env::var("CIRCUIT_BREAKER_RECOVERY_TIMEOUT_SECONDS") {
            self.circuit_breaker.recovery_timeout_seconds = recovery_timeout
                .parse()
        ...
        if let Ok(shutdown_timeout) = env::var("GRACEFUL_SHUTDOWN_SHUTDOWN_TIMEOUT_SECS") {
            self.graceful_shutdown.shutdown_timeout_secs = shutdown_timeout
                .parse()
        ...
        if let Ok(force_shutdown) = env::var("GRACEFUL_SHUTDOWN_FORCE_SHUTDOWN_AFTER_SECS") {
            self.graceful_shutdown.force_shutdown_after_secs = force_shutdown
                .parse()
    ...
    fn validate(&self) -> Result<()> {
        ...
        if !valid_environments.contains(&self.platform.environment.as_str()) {
            anyhow::bail!(
                "Platform environment must be one of: {:?}",
                valid_environments
            );
        }
        ...
        if self.security.enable_tls
            && (self.security.tls_cert_path.is_empty() || self.security.tls_key_path.is_empty())
        {
        ...
        if self.alerts.enable_pagerduty && self.alerts.pagerduty_service_key.is_none() {
            anyhow::bail!(
                "PagerDuty service key must be provided when PagerDuty alerts are enabled"
            );
        }
    ...
    fn validate_model_availability(&self) -> Result<()> {
        ...
        if !self.neural.use_real_models {
            let real_models_configured: Vec<&String> = self
                .neural
                .models
                .iter()
        ...
        let fann_compatible_models: Vec<&String> = self
            .neural
            .models
            .iter()
        ...
        if self.neural.use_real_models {
            if self.neural.model_timeout_seconds < 10 {
                anyhow::bail!(
                    "Neural model_timeout_seconds must be at least 10 seconds for real models"
                );
            }
