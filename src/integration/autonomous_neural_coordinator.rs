...
use crate::integration::daa_coordinator::{DaaCoordinator, AutonomousDecision, TradingAction};
...
impl AutonomousNeuralCoordinator {
    ...
    pub async fn new(
        ...
        let (training_engine, training_receiver) = AutonomousTrainingEngine::new(training_config.clone())?;
        let training_engine = Arc::new(training_engine);
        
        // Create training integration
        let training_integration = DAATrainingIntegration::new(
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let training_integration = self.training_integration.clone();
    ...
    pub async fn make_enhanced_decision(
        ...
        self.evaluate_and_trigger_training(market_context, historical_data).await?;
        
        // Get base decision from DAA coordinator
        let mut decision = self.daa_coordinator
            .make_decision(market_context, current_position, historical_data)
            .await?;
        
        // Enhance decision with neural training insights
        self.enhance_decision_with_training_insights(&mut decision).await?;
        
        // Update performance tracking
        self.update_performance_tracking(&decision, market_context).await?;
    ...
    async fn evaluate_and_trigger_training(
        ...
        let performance_snapshot = self.calculate_performance_snapshot(
            market_context,
            historical_data,
        ).await?;
        
        // Evaluate training need
        let training_decision = self.training_engine
        ...
        match &training_decision.decision_type {
            ...
            _ => {
                info!("Training decision made: {:?} (confidence: {:.2}%)", 
                     training_decision.decision_type, 
                     training_decision.confidence * 100.0);
                info!("Training reasoning: {}", training_decision.reasoning.join(", "));
            }
        }
    ...
    async fn calculate_performance_snapshot(
        ...
        let prediction_accuracy = if performance_tracker.recent_predictions.len() >= 5 {
            let recent_predictions = &performance_tracker.recent_predictions
                [performance_tracker.recent_predictions.len().saturating_sub(10)..];
            
            let total_error: f64 = recent_predictions.iter()
                .map(|(_, actual, predicted)| {
                    let error = (actual - predicted).abs() / actual.abs().max(0.01);
                    if error < 0.1 { 1.0 } else { 0.0 } // Within 10% = success
                })
                .sum();
        ...
        let model_agreement = match self.enhanced_predictor.read().await
            .predict_with_confidence(historical_data, 1).await {
            Ok(predictions) => {
    ...
    async fn enhance_decision_with_training_insights(
        ...
        let recent_training = training_history.values()
            .filter(|record| {
                record.decision.timestamp > Utc::now() - chrono::Duration::hours(24)
            })
            .any(|record| {
        ...
        if recent_training {
            ...
            decision.reasoning.push(
                "Confidence enhanced due to recent successful neural training".to_string()
            );
        }
        
        // Check for ongoing training that might affect decision reliability
        let ongoing_training = training_history.values()
            .any(|record| {
                matches!(record.outcome, 
        ...
        if ongoing_training {
            ...
            decision.reasoning.push(
                "Confidence reduced due to ongoing neural training".to_string()
            );
        }
    ...
    async fn update_performance_tracking(
        ...
        performance_tracker.recent_decisions.push((decision.timestamp, decision.clone()));
        ...
        match &decision.action {
            TradingAction::Buy { .. } | TradingAction::Sell { .. } => {
                ...
                let estimated_profitable = decision.confidence > 0.75 && 
                    market_context.volatility < 0.05;
                ...
                // Update win rate
                trading_perf.win_rate = trading_perf.profitable_trades as f64 / trading_perf.total_trades as f64;
                ...
                if current_pnl < 0.0 {
                    ...
                    trading_perf.max_drawdown = trading_perf.max_drawdown.max(trading_perf.current_drawdown);
                } else {
...
mod tests {
    ...
    async fn create_test_coordinator() -> AutonomousNeuralCoordinator {
        ...
        let daa_coordinator = Arc::new(
            DaaCoordinator::new(daa_config, neural_predictor.clone(), tx).unwrap()
        );
        ...
        AutonomousNeuralCoordinator::new(
            daa_coordinator,
            enhanced_predictor,
            training_config,
        ).await.unwrap()
    ...
    async fn test_performance_tracking() {
        ...
        let snapshot = coordinator.calculate_performance_snapshot(
            &market_context,
            &test_data,
        ).await.unwrap();
    ...
    async fn test_training_evaluation() {
        ...
        let training_decision = coordinator.force_training_evaluation(
            &market_context,
            &test_data,
        ).await.unwrap();
