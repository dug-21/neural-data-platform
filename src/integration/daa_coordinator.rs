...
mod tests {
    ...
    use crate::strategies::{StrategyError, StrategyConfig};
    ...
    async fn test_risk_assessment() {
        ...
        let risk = coordinator.assess_risk(&market_context, None).await.unwrap();
        ...
        // Test with position
        let position = create_test_position();
        let risk_with_position = coordinator.assess_risk(&market_context, Some(&position)).await.unwrap();
