...
impl DataAccessLayer {
    ...
    pub async fn handle_agent_data_request(&self, request: DataRequest) -> Result<DataResponse> {
        ...
        debug!("Handling data request from agent {}: {:?}", request.agent_id, request);

        // Update metrics
        {
            let mut metrics = self.metrics.write().await;
            metrics.total_requests += 1;
            metrics.active_agents.insert(request.agent_id.clone(), Utc::now());
        }
        ...
        let response = match result {
            ...
            Err(e) => {
                error!("Failed to handle request from agent {}: {}", request.agent_id, e);
        ...
        info!("Completed request for agent {} in {}ms", request.agent_id, response.response_time_ms);
        Ok(response)
    }

    /// Get market data for agents
    pub async fn get_market_data(&self, symbol: &str, timeframe: Timeframe) -> Result<Vec<TimeSeriesData>> {
        ...
        let raw_data = self.storage.query_range(symbol, start_time, end_time).await
        ...
        for data_point in raw_data {
            if let Some(metadata) = &data_point.metadata {
                time_series_data.push(TimeSeriesData {
                    ...
                    low: metadata.get("low").and_then(|v| v.as_f64()).unwrap_or(data_point.value),
                    close: data_point.value,
                    volume: metadata.get("volume").and_then(|v| v.as_f64()).unwrap_or(0.0),
        ...
        if let Err(e) = self.cache.set(&cache_key, &time_series_data, Some(ttl)).await {
    ...
    pub async fn get_latest_prices(&self, symbols: Vec<String>) -> Result<PriceMap> {
        ...
        for symbol in symbols {
            ...
            if let Ok(Some(latest_data)) = self.cache.get::<TimeSeriesData>(&cache_key).await {
                price_map.insert(symbol, PriceInfo {
                ...
                if let Ok(data) = self.storage.query_range(&symbol, start_time, end_time).await {
                    if let Some(latest) = data.last() {
                        let price_info = PriceInfo {
                            price: latest.value,
                            timestamp: latest.timestamp,
                            volume: latest.metadata
                                .as_ref()
    ...
    pub async fn get_performance_metrics(&self) -> Result<PerformanceMetrics> {
        ...
        let active_agent_count = metrics.active_agents
            .values()
    ...
    async fn handle_historical_data_request(&self, request: &DataRequest) -> Result<DataResponse> {
        let data = self.get_market_data(&request.symbol, request.timeframe.clone()).await?;
    ...
    async fn handle_latest_prices_request(&self, request: &DataRequest) -> Result<DataResponse> {
        let symbols = if request.symbol.contains(',') {
            request.symbol.split(',').map(|s| s.trim().to_string()).collect()
        } else {
    ...
    async fn handle_aggregated_stats_request(&self, request: &DataRequest) -> Result<DataResponse> {
        let start_time = request.start_time.unwrap_or_else(|| Utc::now() - Duration::hours(24));
        ...
        let stats = self.storage.get_statistics(&request.symbol, start_time, end_time, interval).await?;
        
        // Convert aggregated stats to TimeSeriesData format
        let mut data = Vec::new();
        for stat in stats {
            let mut indicators = HashMap::new();
            if let Some(avg) = stat.avg_value { indicators.insert("avg".to_string(), avg); }
            if let Some(min) = stat.min_value { indicators.insert("min".to_string(), min); }
            if let Some(max) = stat.max_value { indicators.insert("max".to_string(), max); }
            if let Some(stddev) = stat.stddev { indicators.insert("stddev".to_string(), stddev); }
    ...
    async fn handle_subscription_request(&self, request: &DataRequest) -> Result<DataResponse> {
        let subscription_id = format!("{}:{}:{}", request.agent_id, request.symbol, Utc::now().timestamp());
        
        let subscription = AgentSubscription {
            agent_id: request.agent_id.clone(),
            symbol: request.symbol.clone(),
            timeframe: request.timeframe.clone(),
            subscription_type: request.metadata.get("stream_type").cloned().unwrap_or_default(),
            created_at: Utc::now(),
            last_update: Utc::now(),
        };
    ...
    async fn handle_unsubscription_request(&self, request: &DataRequest) -> Result<DataResponse> {
        let subscription_id = request.metadata.get("subscription_id")
        ...
        metadata.insert("status".to_string(), if removed { "removed" } else { "not_found" }.to_string());
        ...
        Ok(DataResponse {
            ...
            data: Vec::new(),
            metadata,
            error_message: if !removed { Some("Subscription not found".to_string()) } else { None },
            response_time_ms: 0,
            data_source: "subscription".to_string(),
        })
    }

    fn create_error_response(&self, request: &DataRequest, error: String, start_time: std::time::Instant) -> DataResponse {
        DataResponse {
            agent_id: request.agent_id.clone(),
            success: false,
            data: Vec::new(),
