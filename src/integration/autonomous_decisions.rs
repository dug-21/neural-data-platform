...
use super::{TradeOrder, OrderSide, OrderType};
...
impl DaaDecisionMaker {
    ...
    pub async fn analyze_market_trend(
        ...
        debug!("Market trend analysis: {:?} with confidence {:?}", trend, confidence);
    ...
    pub async fn make_trading_decision(
        ...
        if let Some(ref o) = order {
            info!(
                ...
                o.side, o.quantity, o.symbol, confidence.value()
            );
        }
    ...
    pub async fn adapt_from_outcome(
        ...
        if recent_decisions.len() >= 5 {
            ...
            debug!("Adapted decision threshold to {:.2} based on {:.1}% success rate",
                self.decision_threshold, success_rate * 100.0);
        }
    ...
    pub async fn get_decision_summary(&self) -> HashMap<String, f64> {
        ...
        let profitable_decisions = history.iter()
            .filter(|d| d.outcome.unwrap_or(0.0) > 0.0)
            .count() as f64;
        
        let avg_confidence = history.iter()
            .map(|d| d.confidence.value())
            .sum::<f64>() / total_decisions.max(1.0);
        
        let total_pnl = history.iter()
            .filter_map(|d| d.outcome)
            .sum::<f64>();
        ...
        HashMap::from([
            ...
            ("win_rate".to_string(), profitable_decisions / total_decisions.max(1.0)),
...
mod tests {
    ...
    async fn test_confidence_level() {
        assert!(matches!(ConfidenceLevel::from_score(0.95), ConfidenceLevel::VeryHigh(_)));
        assert!(matches!(ConfidenceLevel::from_score(0.8), ConfidenceLevel::High(_)));
        assert!(matches!(ConfidenceLevel::from_score(0.65), ConfidenceLevel::Medium(_)));
        assert!(matches!(ConfidenceLevel::from_score(0.45), ConfidenceLevel::Low(_)));
        assert!(matches!(ConfidenceLevel::from_score(0.3), ConfidenceLevel::VeryLow(_)));
    }
