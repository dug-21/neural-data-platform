...
use super::config_utils::{parse_redis_url, parse_postgres_url};
...
impl ConfigBridge {
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn timescale_config_from_platform(platform_config: &PlatformConfig) -> Result<TimescaleConfig> {
    ...
    pub fn adapter_configs_from_platform(platform_config: &PlatformConfig) -> Result<(RedisConfig, TimescaleConfig)> {
...
mod tests {
    use super::*;
    use crate::config::{DatabaseConfig, RedisConfig as PlatformRedisConfig, PlatformInfo, NeuralConfig, MonitoringConfig};
    ...
    fn test_timescale_config_from_platform() {
        let platform_config = create_test_platform_config();
        let timescale_config = ConfigBridge::timescale_config_from_platform(&platform_config).unwrap();
    ...
    fn test_adapter_configs_from_platform() {
        let platform_config = create_test_platform_config();
        let (redis_config, timescale_config) = ConfigBridge::adapter_configs_from_platform(&platform_config).unwrap();
