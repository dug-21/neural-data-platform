...
pub fn parse_redis_url(redis_url: &str) -> Result<RedisConfig> {
    let url = Url::parse(redis_url).context("Failed to parse Redis URL")?;
...
pub fn parse_postgres_url(postgres_url: &str) -> Result<TimescaleConfig> {
    let url = Url::parse(postgres_url).context("Failed to parse PostgreSQL URL")?;

    // Validate scheme
    if url.scheme() != "postgres" && url.scheme() != "postgresql" {
        anyhow::bail!(
            "Invalid PostgreSQL URL scheme: expected 'postgres' or 'postgresql', got '{}'",
            url.scheme()
        );
    }
