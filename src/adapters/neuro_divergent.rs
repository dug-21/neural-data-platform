...
use super::vendor_bridge::{ModelError, PredictionResult, TrainingConfig};
...
impl MockDeepAR {
    ...
    pub async fn fit(
        &mut self,
        _data: &LocalVendorTimeSeriesData,
        _config: &VendorTrainingConfig,
    ) -> Result<(), String> {
        self.trained = true;
        Ok(())
    }
    ...
    pub async fn predict(
        &self,
        data: &LocalVendorTimeSeriesData,
    ) -> Result<VendorPredictionResult, String> {
        if !self.trained {
        ...
        let timestamps = (0..self.config.horizon)
            .map(|i| {
                *data.timestamps.last().unwrap_or(&Utc::now())
...
impl NeuroDivergentAdapter {
    ...
    fn to_vendor_format(
        data: &[TimeSeriesData],
        symbol: &str,
    ) -> Result<LocalVendorTimeSeriesData> {
        if data.is_empty() {
        ...
        if let Some(first_point) = data.first() {
            if !first_point.indicators.is_empty() {
                ...
                for (idx, (_, _)) in first_point.indicators.iter().enumerate() {
                    for point in data {
                        let indicator_values: Vec<f32> =
                            point.indicators.values().map(|&v| v as f32).collect();
        ...
        let vendor_data = LocalVendorTimeSeriesData::new(symbol.to_string(), timestamps, values);
    ...
    pub async fn train_deepar(&mut self, data: &[TimeSeriesData], symbol: &str) -> Result<()> {
        let model = self.deepar_model.as_ref().ok_or_else(|| {
            AdapterError::ModelNotInitialized("DeepAR not initialized".to_string())
        })?;
        ...
        task::spawn_blocking(move || {
            ...
            rt.block_on(async {
                ...
                model_guard
                    .fit(&vendor_data_clone, &training_config_clone)
                    .await
    ...
    pub async fn predict_deepar(
        ...
        let predictions = task::spawn_blocking(move || {
            ...
            rt.block_on(async {
                let mut model_guard = model_clone.lock().await;
                model_guard.predict(&vendor_data_clone).map_err(|e| {
                    AdapterError::Prediction(format!("DeepAR prediction failed: {:?}", e))
                })
            })
        })
        .await??;
        ...
        Ok(Self::from_vendor_predictions(
            &vendor_predictions,
            &symbol_clone,
        ))
    }
    ...
    pub async fn predict_tcn(
        ...
        let predictions = task::spawn_blocking(move || {
            ...
            rt.block_on(async {
                let mut model_guard = model_clone.lock().await;
                model_guard.predict(&vendor_data_clone).map_err(|e| {
                    AdapterError::Prediction(format!("TCN prediction failed: {:?}", e))
                })
            })
        })
        .await??;
...
// use super::vendor_bridge::*; // Unused import
...
impl VendorDeepAR {
    pub fn new(
        ...
        let network_builder = ruv_fann::NetworkBuilder::new();
        ...
        let network = network_builder.layers_from_sizes(&layers).build();
    ...
    pub async fn fit(
        ...
        if !training_data.is_empty() {
            ...
            for (input, target) in training_data.iter().take(100) {
    ...
    pub fn predict(
        ...
        metadata.insert(
            ...
            "ruv_fann_neural_network".to_string(),
        );
...
impl VendorNHITS {
    ...
    pub fn predict(
        ...
        for &resolution in &resolutions {
            let downsampled: Vec<f32> = last_values.iter().step_by(resolution).copied().collect();
        ...
        for i in 0..self.horizon {
            let combined = multi_res_forecasts
                .iter()
                .map(|forecasts| forecasts.get(i).copied().unwrap_or(0.0))
                .sum::<f32>()
                / multi_res_forecasts.len() as f32;
...
impl VendorDDPM {
    ...
    pub async fn predict(
        ...
        let timestamps = (0..24)
            .map(|i| {
                *data.timestamps.last().unwrap_or(&Utc::now())
                    + chrono::Duration::hours(i as i64 + 1)
            })
            .collect();
