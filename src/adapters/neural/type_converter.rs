...
impl SafeF32Convert for f64 {
    fn to_f32_safe(&self) -> Result<f32, NeuralAdapterError> {
        ...
        if self.is_infinite() {
            return Ok(if self.is_sign_positive() { f32::INFINITY } else { f32::NEG_INFINITY });
        }
...
pub fn convert_indicators_to_f32(indicators: &HashMap<String, f64>) -> Result<HashMap<String, f32>, NeuralAdapterError> {
    indicators
        .iter()
        .map(|(key, &value)| {
            value.to_f32_safe().map(|v| (key.clone(), v))
        })
        .collect()
}
...
impl VendorDataConverter {
    ...
    pub fn validate_precision(&self, original: f64, converted: f64) -> Result<(), NeuralAdapterError> {
        ...
        if original == 0.0 {
            ...
            if converted.abs() > f64::EPSILON {
                return Err(NeuralAdapterError::Conversion(
                    format!("Non-zero conversion from zero: {}", converted)
                ));
            }
            return Ok(());
        }
    ...
    pub fn to_ruv_swarm_format(&self, data: &[TimeSeriesData], symbol: &str) -> Result<RuvSwarmTimeSeriesData, NeuralAdapterError> {
        if data.is_empty() {
    ...
    pub fn to_neuro_divergent_datapoints(&self, data: &[TimeSeriesData]) -> Result<Vec<NeuralDataPoint>, NeuralAdapterError> {
        data.iter()
            .map(|point| {
                ...
                let exogenous = if point.indicators.is_empty() {
                    ...
                    let exog_values = point.indicators.values()
                        .map(|&v| v.to_f32_safe())
                        .collect::<Result<Vec<f32>, _>>()?;
                    Some(exog_values)
                };
    ...
    pub fn validate_precision_f32(&self, original: f64, converted: f32) -> Result<(), NeuralAdapterError> {
...
impl BatchConverter {
    ...
    pub fn convert_multi_symbol(
        ...
        data_by_symbol
            .iter()
            .map(|(symbol, data)| {
                converter.to_ruv_swarm_format(data, symbol)
    ...
    pub fn verify_conversions(
        ...
        for (symbol, orig_data) in original {
            let conv_data = converted.get(symbol)
                .ok_or_else(|| NeuralAdapterError::Conversion(format!("Missing conversion for symbol: {}", symbol)))?;
                
            if orig_data.len() != conv_data.values.len() {
                return Err(NeuralAdapterError::Conversion(format!(
                    "Length mismatch for {}: {} != {}", 
                    symbol, orig_data.len(), conv_data.values.len()
                )));
            }
        }
        Ok(())
...
impl StreamingConverter {
    ...
    pub fn convert_chunked(
        ...
        let frequency = self.converter.infer_frequency(data)
            .unwrap_or_else(|| "1H".to_string());
