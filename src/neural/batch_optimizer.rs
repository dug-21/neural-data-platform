...
impl BatchOptimizer {
    ...
    pub async fn predict_batch(
        ...
        for batch in batches {
            let batch_results = self
                .process_batch_parallel(predictor, model_name, batch, horizon)
                .await?;
    ...
    pub fn extract_features_parallel(
        ...
        data_windows
            .par_iter()
            .map(|window| self.extract_features_single(window, lookback))
            .collect()
    }

    /// Extract features for a single data window
    fn extract_features_single(&self, data: &[TimeSeriesData], lookback: usize) -> Vec<f32> {
...
impl BatchPerformanceMonitor {
    ...
    pub fn report(&self) {
        ...
        info!("Average batch time: {:.2}ms", self.get_average_batch_time());
    }
}
