...
mod latency_performance_tests {
    ...
    async fn test_single_prediction_latency() -> Result<()> {
        ...
        assert!(
            ...
            "Single prediction took too long: {:?}",
            duration
        );
    ...
    async fn test_ensemble_prediction_latency() -> Result<()> {
        ...
        let start = Instant::now();
        let predictions = fann_predictor
            .predict_ensemble(&test_data, 8, &models, None)
            .await?;
        let duration = start.elapsed();
        ...
        assert!(
            ...
            "Ensemble prediction took too long: {:?}",
            duration
        );
...
mod throughput_performance_tests {
    ...
    async fn test_concurrent_prediction_throughput() -> Result<()> {
        ...
        println!(
            "✅ Concurrent throughput: {:.2} predictions/second",
            throughput
        );
    ...
    async fn test_sustained_load_performance() -> Result<()> {
        ...
        println!(
            "✅ Sustained load - Overall throughput: {:.2} pred/sec",
            overall_throughput
        );
        println!(
            "✅ Sustained load - Success rate: {:.2}%",
            success_rate * 100.0
        );
...
mod memory_performance_tests {
    ...
    async fn test_memory_usage_with_large_datasets() -> Result<()> {
        ...
        for size in dataset_sizes {
            ...
            println!(
                "Dataset size: {}, Memory usage: {:.2} MB",
                size, current_memory
            );
    ...
    async fn test_memory_efficiency_with_caching() -> Result<()> {
        ...
        assert!(
            duration2 <= duration1,
            "Cache not working - second prediction slower"
        );
        ...
        assert!(
            cache_memory_increase < 50.0,
            "Cache using too much memory: {:.2} MB",
...
mod scalability_tests {
    ...
    async fn test_scalability_with_increasing_horizons() -> Result<()> {
        ...
        for horizon in horizons {
            ...
            match predictor.predict_with_confidence(&test_data, horizon).await {
                Ok(predictions) => {
                    ...
                    println!(
                        "Horizon: {}, Duration: {:?}, Throughput: {:.2} pred/sec",
        ...
        assert!(
            performance_metrics.len() >= 3,
            "Too many horizon tests failed"
        );
        ...
        for (horizon, duration, throughput) in &performance_metrics {
            assert!(
                duration.as_secs() < 10,
                "Horizon {} took too long: {:?}",
                horizon,
                duration
            );
            assert!(
                *throughput > 0.5,
                "Throughput too low for horizon {}: {:.2}",
                horizon,
                throughput
            );
        }
    ...
    async fn test_scalability_with_model_count() -> Result<()> {
        ...
        for models in model_sets {
            ...
            match fann_predictor
                .predict_ensemble(&test_data, 5, &models, None)
                .await
            {
                Ok(predictions) => {
                    let duration = start.elapsed();
        ...
        assert!(
            ensemble_metrics.len() >= 2,
            "Too many ensemble tests failed"
        );
...
mod stress_tests {
    ...
    async fn test_high_frequency_requests() -> Result<()> {
        ...
        println!(
            "✅ High frequency test - Success rate: {:.2}%",
            success_rate * 100.0
        );
        println!(
            "✅ High frequency test - Throughput: {:.2} pred/sec",
            throughput
        );
    ...
    async fn test_error_recovery_performance() -> Result<()> {
        ...
        let result = predictor
            .predict_with_confidence(&problematic_data, 3)
            .await;
...
mod cache_performance_tests {
    ...
    async fn test_cache_memory_efficiency() -> Result<()> {
        ...
        println!(
            "✅ Memory increase with {} cached datasets: {:.2} MB",
            num_datasets, memory_increase
        );
...
async fn test_comprehensive_performance_benchmark() -> Result<()> {
    ...
    let start = Instant::now();
    let enhanced_predictions = enhanced_predictor
        .predict_with_confidence(&test_data, 10)
        .await?;
    ...
    let models = vec!["MLP".to_string(), "NHITS".to_string(), "DeepAR".to_string()];
    let start = Instant::now();
    let ensemble_predictions = fann_predictor
        .predict_ensemble(&test_data, 10, &models, None)
        .await?;
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            predictor_clone
                .predict_with_confidence(&data_clone, 5 + (i % 3))
                .await
        });
        handles.push(handle);
    }
    ...
    let enhanced_avg_confidence = enhanced_predictions
        .iter()
        .map(|p| p.confidence)
        .sum::<f64>()
        / enhanced_predictions.len() as f64;

    let ensemble_avg_confidence = ensemble_predictions
        .iter()
        .map(|p| p.confidence)
        .sum::<f64>()
    ...
    assert!(
        enhanced_throughput > 1.0,
        "Enhanced predictor throughput too low"
    );
