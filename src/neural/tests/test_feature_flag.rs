...
async fn test_feature_flag_disabled_fann_only() -> Result<()> {
    ...
    for model_name in &["MLP", "LSTM", "DeepAR"] {
        ...
        println!("✅ FANN-only prediction for '{}' successful: {} predictions", model_name, predictions.len());
    }
    
    // Test ensemble predictions
    let ensemble_predictions = predictor.predict_ensemble(
        &test_data, 
        5, 
    ...
    assert!(ensemble_predictions.iter().all(|p| p.confidence > 0.0 && p.confidence <= 1.0));
    assert!(ensemble_predictions.iter().all(|p| p.model_name.contains("ensemble")));
    
    println!("✅ FANN-only ensemble prediction successful: {} predictions", ensemble_predictions.len());
...
async fn test_feature_flag_enabled_with_fallback() -> Result<()> {
    ...
    let config = NeuralConfig {
        memory_gb: 1.0,
        models: vec!["TimeMixer".to_string(), "LSTM".to_string(), "DeepAR".to_string()],
        prediction_cache_ttl: 300,
        model_load_timeout: 60,
        max_concurrent_predictions: 10,
        enable_model_monitoring: true,
    ...
    let predictions = predictor.test_predict_with_model("TimeMixer", &test_data, 5).await?;
    
    assert_eq!(predictions.len(), 5);
    assert!(predictions.iter().all(|p| p.confidence > 0.0 && p.confidence <= 1.0));
    
    println!("✅ Real model fallback prediction for 'TimeMixer' successful: {} predictions", predictions.len());
    
    // Test DeepAR (should try real model first, then fallback)
    let deepar_predictions = predictor.test_predict_with_model("DeepAR", &test_data, 5).await?;
    
    assert_eq!(deepar_predictions.len(), 5);
    assert!(deepar_predictions.iter().all(|p| p.confidence > 0.0 && p.confidence <= 1.0));
    
    println!("✅ DeepAR fallback prediction successful: {} predictions", deepar_predictions.len());
    
    // Test ensemble with mixed model types
    let ensemble_predictions = predictor.predict_ensemble(
        &test_data, 
        5, 
        &["TimeMixer".to_string(), "LSTM".to_string(), "DeepAR".to_string()],
        None
    ).await?;
    ...
    println!("✅ Mixed model ensemble prediction successful: {} predictions", ensemble_predictions.len());
...
async fn test_feature_flag_enhanced_adapter_available() -> Result<()> {
    ...
    if adapter_status.is_some() {
        ...
        if let Ok(predictions) = predictor.test_predict_with_enhanced_model("TimeMixer", &test_data, 5).await {
            assert_eq!(predictions.len(), 5);
            assert!(predictions.iter().all(|p| p.confidence > 0.0 && p.confidence <= 1.0));
            assert!(predictions.iter().any(|p| p.model_name.contains("enhanced")));
            
            println!("✅ Enhanced TimeMixer prediction successful: {} predictions", predictions.len());
        } else {
...
async fn test_ensemble_statistics_with_feature_flag() -> Result<()> {
    ...
    let predictions = predictor.predict_ensemble(
        &test_data,
        5,
    ...
    predictor.update_performance("LSTM", &actual_values, &predictions).await?;
...
async fn test_model_routing_logging() -> Result<()> {
    ...
    let predictions = predictor.test_predict_with_model("TimeMixer", &test_data, 3).await?;
    assert!(!predictions.is_empty());
    
    // This should log using FANN directly (not a real model)
    let mlp_predictions = predictor.test_predict_with_model("MLP", &test_data, 3).await?;
