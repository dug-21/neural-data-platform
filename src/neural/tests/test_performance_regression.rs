...
async fn test_single_prediction_latency() -> Result<()> {
    ...
    let base_predictor = Arc::new(FannPredictor::new(config, false).await?);
    ...
    println!("Single prediction latencies - P50: {}ms, P95: {}ms, P99: {}ms", p50, p95, p99);
    
    // Assert performance requirements
    assert!(p50 <= MAX_SINGLE_PREDICTION_LATENCY_MS, 
            "P50 latency {}ms exceeds max {}ms", p50, MAX_SINGLE_PREDICTION_LATENCY_MS);
    assert!(p95 <= MAX_SINGLE_PREDICTION_LATENCY_MS * 2, 
            "P95 latency {}ms exceeds 2x max", p95);
...
async fn test_batch_throughput() -> Result<()> {
    ...
    assert!(throughput >= MIN_BATCH_THROUGHPUT,
            "Throughput {:.2} below minimum {:.2}", throughput, MIN_BATCH_THROUGHPUT);
...
async fn test_memory_efficiency() -> Result<()> {
    ...
    assert!(memory_growth <= MAX_MEMORY_GROWTH_MB,
            "Memory growth {:.2}MB exceeds max {:.2}MB", memory_growth, MAX_MEMORY_GROWTH_MB);
...
async fn test_parallel_scaling() -> Result<()> {
    ...
    for &batch_size in &batch_sizes {
        ...
        println!("Batch size {}: {:.2} predictions/sec, efficiency: {:.2}", 
                 batch_size, predictions_per_sec, efficiency);
    }
    
    // Verify scaling efficiency
    let avg_efficiency = efficiencies.iter().sum::<f64>() / efficiencies.len() as f64;
    assert!(avg_efficiency >= 0.7, "Poor parallel scaling efficiency: {:.2}", avg_efficiency);
...
async fn test_cache_effectiveness() -> Result<()> {
    ...
    println!("Cache speedup: {:.2}x (cold: {:?}, warm: {:?})", speedup, cold_time, warm_time);
    
    assert!(speedup >= 2.0, "Insufficient cache speedup: {:.2}x", speedup);
    
    // Check cache hit rate
    let metrics = optimized.get_metrics();
    assert!(metrics.cache_hit_rate >= 0.8, "Low cache hit rate: {:.2}", metrics.cache_hit_rate);
...
async fn test_model_preloading() -> Result<()> {
    ...
    let base_predictor = Arc::new(FannPredictor::new(config.clone(), false).await?);
    let optimized = OptimizedFannPredictor::new(base_predictor).await?;
    ...
    println!("Preload time: {:?}, First prediction: {:?}", preload_time, first_predict_time);
    
    // First prediction should be fast due to preloading
    assert!(first_predict_time.as_millis() < 100, 
            "First prediction too slow: {:?}", first_predict_time);
...
async fn test_concurrent_predictions() -> Result<()> {
    ...
    for _ in 0..concurrent_tasks {
        ...
        let handle = tokio::spawn(async move {
            for _ in 0..10 {
                let _ = optimized_clone.predict_batch("MLP", vec![data_clone.as_slice()], 5).await?;
            }
            Result::<()>::Ok(())
        });
    ...
    println!("Concurrent predictions: {} tasks, {:.2} predictions/sec", 
             concurrent_tasks, predictions_per_sec);
    
    assert!(predictions_per_sec >= MIN_BATCH_THROUGHPUT / 2.0,
            "Poor concurrent performance: {:.2}", predictions_per_sec);
