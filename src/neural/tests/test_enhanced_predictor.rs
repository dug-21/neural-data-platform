...
mod core_functionality_tests {
    ...
    async fn test_predict_with_confidence_basic() -> Result<()> {
        ...
        for (step, result) in results.iter().enumerate() {
            ...
            assert!(breakdown.market_regime_adjustment >= -0.1 && breakdown.market_regime_adjustment <= 0.1);
            ...
            if step > 0 {
                assert!(breakdown.temporal_distance_penalty <= results[0].confidence_breakdown.temporal_distance_penalty);
            }
        }
    ...
    async fn test_predict_with_confidence_volatile_data() -> Result<()> {
        ...
        let results = predictor
            .predict_with_confidence(&volatile_data, 4)
            .await?;
    ...
    async fn test_predict_with_confidence_bearish_market() -> Result<()> {
        ...
        let results = predictor
            .predict_with_confidence(&bearish_data, 3)
            .await?;
...
mod retraining_logic_tests {
    ...
    async fn test_should_retrain_accuracy_below_threshold() -> Result<()> {
        ...
        predictor.update_performance(&actual_values, &predicted_results).await?;
        ...
        if metrics.current_accuracy < 0.7 {
            ...
            assert!(metrics.retrain_reasons.iter().any(|r| r.contains("Accuracy below threshold")));
        }
    ...
    async fn test_should_retrain_samples_threshold() -> Result<()> {
        ...
        assert!(metrics.urgency_score > 0.0);
        assert!(metrics.retrain_reasons.iter().any(|r| r.contains("New samples available")));
...
mod performance_tracking_tests {
    ...
    async fn test_update_performance_basic() -> Result<()> {
        ...
        let result = predictor.update_performance(&actual_values, &predicted_results).await;
        assert!(result.is_ok());
    ...
    async fn test_mark_retrained() -> Result<()> {
        ...
        let samples_before = metrics_before.get("new_samples_count").unwrap().as_u64().unwrap();
        ...
        let samples_after = metrics_after.get("new_samples_count").unwrap().as_u64().unwrap();
    ...
    async fn test_get_performance_metrics() -> Result<()> {
        ...
        let total_preds = metrics.get("total_predictions").unwrap().as_u64().unwrap();
        let successful_preds = metrics.get("successful_predictions").unwrap().as_u64().unwrap();
        ...
        match result {
            Ok(results) => {
                ...
                for (i, result) in results.iter().enumerate() {
                    ...
                    if i > 0 && i < 10 { // Check first 10 to avoid floating point accumulation issues
                        assert!(result.confidence_breakdown.temporal_distance_penalty <= 
...
mod integration_tests {
    ...
    async fn test_full_prediction_workflow() -> Result<()> {
        ...
        let actual_values = vec![100.0, 101.0, 102.0, 103.0, 104.0];
        predictor.update_performance(&actual_values, &results).await?;
    ...
    async fn test_confidence_breakdown_integration() -> Result<()> {
        ...
        for result in &results {
            ...
            assert_abs_diff_eq!(breakdown.combined_confidence, clamped_manual, epsilon = 0.001);
            
            // Final confidence should be close to combined confidence
            assert_abs_diff_eq!(result.confidence, breakdown.combined_confidence, epsilon = 0.1);
        }
