    ...
    (0..count)
        .map(|i| {
            ...
            TimeSeriesData {
                ...
                volume: vec![1000000.0 + (i as f64 * 10000.0)],
                indicators,
...
mod fann_prediction_tests {
    ...
    async fn test_ensemble_with_all_models() -> Result<()> {
        ...
        let results = predictor
            .predict_ensemble(&test_data, 8, &all_models, None)
            .await?;
...
mod market_regime_tests {
    ...
    async fn test_bullish_market_detection() -> Result<()> {
        ...
        let _results = predictor
            .predict_ensemble(&bullish_data, 3, &models, None)
            .await?;
    ...
    async fn test_high_volatility_detection() -> Result<()> {
        ...
        let _results = predictor
            .predict_ensemble(&volatile_data, 3, &models, None)
            .await?;
    ...
    async fn test_bearish_market_detection() -> Result<()> {
        ...
        let _results = predictor
            .predict_ensemble(&bearish_data, 3, &models, None)
            .await?;
...
mod dynamic_weighting_tests {
    ...
    async fn test_dynamic_weight_adjustment() -> Result<()> {
        ...
        let initial_weights = initial_stats
            .get("dynamic_weights")
            .unwrap()
            .as_object()
            .unwrap();
        ...
        for i in 0..15 {
            ...
            let results = predictor
                .predict_ensemble(&current_data, 3, &models, None)
                .await?;

            // Simulate performance feedback
            let actual_values = vec![current_data.last().unwrap().close; 3];
            predictor
                .update_performance("MLP", &actual_values, &results)
                .await?;
        }

        // Get updated weights
        let updated_stats = predictor.get_ensemble_stats().await?;
        let updated_weights = updated_stats
            .get("dynamic_weights")
            .unwrap()
            .as_object()
            .unwrap();
    ...
    async fn test_performance_based_weight_adjustment() -> Result<()> {
        ...
        predictor
            .update_performance("MLP", &actual_values, &good_predictions)
            .await?;
        predictor
            .update_performance("NHITS", &actual_values, &poor_predictions)
            .await?;

        // Check that performance metrics reflect the updates
        let stats = predictor.get_ensemble_stats().await?;
        let model_performances = stats
            .get("model_performances")
            .unwrap()
            .as_object()
            .unwrap();
    ...
    async fn test_ensemble_reset_functionality() -> Result<()> {
        ...
        let models = vec!["MLP".to_string(), "DeepAR".to_string()];
        let results = predictor
            .predict_ensemble(&test_data, 3, &models, None)
            .await?;
        ...
        let performances_before = stats_before
            .get("model_performances")
            .unwrap()
            .as_object()
            .unwrap();
        ...
        let performances_after = stats_after
            .get("model_performances")
            .unwrap()
            .as_object()
            .unwrap();
        ...
        for (feature, importance) in &feature_importance {
            assert!(
                *importance >= 0.0 && *importance <= 1.0,
                "Feature {} has invalid importance: {}",
                feature,
                importance
            );
        }
...
mod error_handling_tests {
    ...
    async fn test_prediction_with_invalid_models() -> Result<()> {
        ...
        let result = predictor
            .predict_ensemble(&test_data, 3, &invalid_models, None)
            .await;
        ...
        match result {
            ...
            Err(err) => {
                ...
                assert!(
                    err.to_string().contains("model") || err.to_string().contains("prediction")
                );
            }
        }
    ...
    async fn test_ensemble_with_no_models() -> Result<()> {
        ...
        let result = predictor
            .predict_ensemble(&test_data, 3, &no_models, None)
            .await;
...
mod performance_tracking_tests {
    ...
    async fn test_model_performance_tracking() -> Result<()> {
        ...
        // Update performance
        predictor
            .update_performance("MLP", &actual_values, &predictions)
            .await?;
...
mod integration_verification_tests {
    ...
    async fn test_comprehensive_workflow() -> Result<()> {
        ...
        let models = vec!["MLP".to_string(), "NHITS".to_string(), "DeepAR".to_string()];
        let ensemble_results = predictor
            .predict_ensemble(&test_data, 5, &models, None)
            .await?;
        ...
        predictor
            .update_performance("MLP", &actual_values, &single_results)
            .await?;

        // 4. Test online learning
        predictor
            .update_with_new_data("MLP", &test_data[20..30])
            .await?;
...
async fn test_fann_predictor_coverage_verification() -> Result<()> {
    ...
    let models = vec!["MLP".to_string(), "NHITS".to_string()];
    let _ensemble_results = predictor
        .predict_ensemble(&test_data, 3, &models, None)
        .await?;
    ...
    predictor
        .update_performance("MLP", &actual, &predictions)
        .await?;

    // 5. update_with_new_data (public method)
    predictor
        .update_with_new_data("MLP", &test_data[0..10])
        .await?;
