        ...
        group.bench_with_input(
            ...
            |b, &model_name| {
                b.to_async(&runtime).iter(|| async {
                    ...
                    let predictor = FannPredictor::new(config, false).await.unwrap();
...
fn benchmark_prediction_latency(c: &mut Criterion) {
    ...
    let predictor = runtime.block_on(async {
        let config = NeuralConfig::default();
        FannPredictor::new(config, false).await.unwrap()
    });
        ...
        for horizon in &horizons {
            ...
            group.bench_with_input(
                ...
                |b, (data, horizon)| {
                    b.to_async(&runtime).iter(|| async {
                        black_box(
                            predictor.predict("MLP", data, *horizon).await.unwrap()
                        );
                    });
                },
            );
...
fn benchmark_batch_throughput(c: &mut Criterion) {
    ...
    let predictor = runtime.block_on(async {
        let config = NeuralConfig::default();
        let base = std::sync::Arc::new(FannPredictor::new(config, false).await.unwrap());
    ...
    for batch_size in &batch_sizes {
        ...
        group.bench_with_input(
            ...
            |b, batch| {
                b.to_async(&runtime).iter(|| async {
                    black_box(
                        predictor.predict_batch("MLP", batch.clone(), 5).await.unwrap()
                    );
                });
            },
        );
...
fn benchmark_cache_efficiency(c: &mut Criterion) {
    ...
    let unique_data: Vec<_> = (0..100)
        .map(|i| generate_benchmark_data(100 + i))
        .collect();
    ...
    group.bench_function("cache_misses", |b| {
        ...
        b.to_async(&runtime).iter(|| async {
            ...
            idx += 1;
            black_box(
                predictor.predict_batch("MLP", vec![data.as_slice()], 5).await.unwrap()
            );
        });
    });
    ...
    group.bench_function("cache_hits", |b| {
        b.to_async(&runtime).iter(|| async {
            black_box(
                predictor.predict_batch("MLP", vec![repeated_data.as_slice()], 5).await.unwrap()
            );
        });
    });
...
fn benchmark_ensemble_performance(c: &mut Criterion) {
    ...
    for size in &ensemble_sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("ensemble", size),
            &models,
            |b, models| {
                b.to_async(&runtime).iter(|| async {
                    black_box(
                        predictor.ensemble_predict(models, &data, 5).await.unwrap()
                    );
                });
            },
        );
