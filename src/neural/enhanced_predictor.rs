...
impl EnhancedNeuralPredictor {
    ...
    pub fn new(config: NeuralConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            market_regime_cache: Arc::new(RwLock::new(("unknown".to_string(), Utc::now() - Duration::hours(1)))),
    ...
    pub async fn predict_with_confidence(
        ...
        let ensemble_predictions = self.fann_predictor
        ...
        let mut predictions_by_timestamp: HashMap<DateTime<Utc>, Vec<&PredictionResult>> = HashMap::new();
        ...
        for i in 0..horizon {
            if let Some(timestamp_predictions) = predictions_by_timestamp
                .values()
                .find(|preds| preds.len() > i && preds[0].timestamp == ensemble_predictions.get(i * self.config.models.len()).map(|p| p.timestamp).unwrap_or(Utc::now()))
            {
                let enhanced_result = self.calculate_enhanced_prediction(
        ...
        if enhanced_results.is_empty() {
            for (i, chunk) in ensemble_predictions.chunks(self.config.models.len()).enumerate() {
                if i >= horizon { break; }
    ...
    async fn calculate_enhanced_prediction(
        ...
        for prediction in predictions {
            let weight = ensemble_weights.get(&prediction.model_name).copied().unwrap_or(1.0);
        ...
        let base_confidence = predictions.iter()
            .zip(predictions.iter().map(|p| ensemble_weights.get(&p.model_name).copied().unwrap_or(1.0)))
            .map(|(p, w)| p.confidence * w)
            .sum::<f64>() / total_weight.max(1.0);
        
        // Calculate confidence breakdown components
        let confidence_breakdown = self.calculate_confidence_breakdown(
            base_confidence,
            agreement_score,
            step,
            volatility,
    ...
    pub async fn should_retrain(&self) -> Result<RetrainingMetrics> {
        ...
        let hours_since_training = (Utc::now() - performance_tracker.last_training_time).num_hours();
        let new_samples = performance_tracker.new_samples_count.load(Ordering::Relaxed);
        ...
        if accuracy_trigger {
            retrain_reasons.push(format!("Accuracy below threshold: {:.3} < {:.3}", 
                current_accuracy, performance_tracker.accuracy_threshold));
        }
        if time_trigger {
            retrain_reasons.push(format!("Time since training: {} hours > {} threshold", 
                hours_since_training, performance_tracker.hours_threshold));
        }
        if samples_trigger {
            retrain_reasons.push(format!("New samples available: {} > {} threshold", 
        ...
        let (primary_trigger, urgency_score) = if accuracy_trigger {
            ...
            let urgency = (hours_since_training as f64 / performance_tracker.hours_threshold as f64) - 1.0;
    ...
    pub async fn update_performance(
        ...
        for (actual, predicted) in actual_values.iter().zip(predicted_results.iter()) {
            ...
            {
                let performance_tracker = self.performance_tracker.read().await;
                performance_tracker.total_predictions.fetch_add(1, Ordering::Relaxed);
                if is_successful {
                    performance_tracker.successful_predictions.fetch_add(1, Ordering::Relaxed);
                }
            }
            ...
            // Update accuracy and history in separate scope
            {
                ...
                performance_tracker.recent_accuracy = 
                    performance_tracker.recent_accuracy * performance_tracker.decay_factor +
                    new_accuracy * (1.0 - performance_tracker.decay_factor);
                    
                // Add to prediction history in nested scope
                {
                    let mut prediction_history = performance_tracker.prediction_history.write().await;
                ...
                debug!("Updated performance: accuracy={:.3}, error={:.3}, success={}",
        ...
        for predicted in predicted_results {
            ...
            info!("Performance update for ensemble size: {}", predicted.ensemble_size);
        }
    ...
    pub async fn add_training_samples(&self, sample_count: usize) -> Result<()> {
        let performance_tracker = self.performance_tracker.read().await;
        performance_tracker.new_samples_count.fetch_add(sample_count, Ordering::Relaxed);
        
        debug!("Added {} training samples, total new samples: {}", 
               sample_count, 
               performance_tracker.new_samples_count.load(Ordering::Relaxed));
    ...
    pub async fn mark_retrained(&self) -> Result<()> {
        ...
        performance_tracker.new_samples_count.store(0, Ordering::Relaxed);
        
        info!("Marked model as retrained at {}", performance_tracker.last_training_time);
        Ok(())
    }
    ...
    pub async fn get_performance_metrics(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        let total_preds = performance_tracker.total_predictions.load(Ordering::Relaxed);
        let successful_preds = performance_tracker.successful_predictions.load(Ordering::Relaxed);
        ...
        metrics.insert("recent_accuracy".to_string(), serde_json::json!(performance_tracker.recent_accuracy));
        metrics.insert("overall_accuracy".to_string(), serde_json::json!(overall_accuracy));
        metrics.insert("total_predictions".to_string(), serde_json::json!(total_preds));
        metrics.insert("successful_predictions".to_string(), serde_json::json!(successful_preds));
        metrics.insert("hours_since_training".to_string(), 
                      serde_json::json!((Utc::now() - performance_tracker.last_training_time).num_hours()));
        metrics.insert("new_samples_count".to_string(), 
                      serde_json::json!(performance_tracker.new_samples_count.load(Ordering::Relaxed)));
        metrics.insert("prediction_history_size".to_string(), serde_json::json!(prediction_history.len()));
        
        Ok(metrics)
    }
    
    /// Calculate current volatility from recent data
    async fn calculate_current_volatility(&self, data: &[TimeSeriesData]) -> Result<f64> {
        if data.len() < 20 {
    ...
    async fn detect_market_regime(&self, data: &[TimeSeriesData]) -> Result<String> {
        // Check cache first
        {
    ...
    /// Update data quality metrics
    async fn update_data_quality(&self, data: &[TimeSeriesData]) -> Result<()> {
        ...
        let latest_timestamp = data.iter()
            .map(|d| d.timestamp)
            .max()
            .unwrap_or(Utc::now());
...
mod tests {
    ...
    async fn test_confidence_calculation() {
        ...
        let predictions = predictor.predict_with_confidence(&test_data, 5).await.unwrap();
    ...
    async fn test_performance_tracking() {
        ...
        predictor.update_performance(&actual_values, &predictions).await.unwrap();
    ...
    async fn test_volatility_calculation() {
        ...
        let volatility = predictor.calculate_current_volatility(&test_data).await.unwrap();
    ...
    async fn test_mark_retrained() {
        ...
        let new_samples = performance_metrics.get("new_samples_count").unwrap().as_u64().unwrap();
        assert_eq!(new_samples, 0);
        
        let metrics_after = predictor.should_retrain().await.unwrap();
        assert!(!metrics_after.should_retrain || metrics_after.primary_trigger == "accuracy_degradation");
    }
}