...
use super::{FannPredictor, FannModelConfig};
...
impl OptimizedFannPredictor {
    ...
    async fn preload_models(&self) -> Result<()> {
        ...
        let tasks: Vec<_> = common_models.into_iter()
            .map(|model_name| {
        ...
        info!("Preloaded {} models in {:.2}ms", 
              self.model_cache.len(), load_time);
    ...
    async fn load_and_cache_model(
        ...
        cache.insert(model_name.to_string(), CachedModel {
    ...
    fn predict_single_optimized(
        ...
        if let Some(cached) = self.prediction_cache.get(&cache_key) {
            ...
            metrics.cache_hit_rate = 
                (metrics.cache_hit_rate * 0.99) + 0.01; // Moving average
    ...
    async fn process_batch(&self, batch: &mut Vec<BatchRequest>) {
        ...
        let tasks: Vec<_> = grouped.into_iter()
            .map(|(model_name, requests)| {
                let predictor = self.clone();
                tokio::spawn(async move {
                    predictor.process_model_batch(model_name, requests).await
                })
            })
            .collect();
    ...
    fn build_prediction_result(
        ...
        let predictions: Vec<f64> = output.iter()
            .take(horizon)
    ...
    fn predict_batch_request(
        ...
        for window in data.windows(config.input_size / 5) {
            let result = self.predict_single_optimized(
                network,
                config,
                window,
                horizon,
...
impl MemoryPool {
    ...
    fn get_input_buffer(&self) -> Vec<f32> {
        self.input_buffers.blocking_lock()
            .pop()
