...
fn main() {
    ...
    let mut training_data = TrainingData::new(2, 1);
    for (input, output) in inputs.iter().zip(&outputs) {
        training_data.add_sample(input.clone(), output.clone());
    }
    
    // Train the network
    let trainer = training_data.create_trainer(&mut trainable_network);
    trainer.train(0.01, 1000, 0); // learning_rate, max_epochs, report_interval
    
    println!("\nAfter training:");
    for (input, expected) in inputs.iter().zip(&outputs) {
        let prediction = trainable_network.run(input);
        let error = (prediction[0] - expected[0]).abs();
        println!("  {} XOR {} = {:.3} (expected: {}, error: {:.3})", 
