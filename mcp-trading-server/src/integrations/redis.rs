...
impl RedisClient {
    pub async fn new(redis_url: &str) -> Result<Self> {
        ...
        let client = Client::open(redis_url).map_err(|e| Error::Redis(e))?;
    ...
    pub async fn get<T: for<'de> Deserialize<'de>>(&mut self, key: &str) -> Result<Option<T>> {
        ...
        let value: Option<String> = self.conn.get(key).await.map_err(|e| Error::Redis(e))?;

        match value {
            Some(json_str) => {
                let data = serde_json::from_str(&json_str).map_err(|e| Error::Serialization(e))?;
                Ok(Some(data))
            }
            None => Ok(None),
        }
    ...
    pub async fn set<T: Serialize>(&mut self, key: &str, value: &T, ttl: Duration) -> Result<()> {
        ...
        let json_str = serde_json::to_string(value).map_err(|e| Error::Serialization(e))?;

        self.conn
            .set_ex(key, json_str, ttl.as_secs())
            .await
            .map_err(|e| Error::Redis(e))?;
    ...
    pub async fn delete(&mut self, key: &str) -> Result<()> {
        ...
        self.conn.del(key).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn exists(&mut self, key: &str) -> Result<bool> {
        ...
        let exists: bool = self.conn.exists(key).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn get_ttl(&mut self, key: &str) -> Result<Option<Duration>> {
        ...
        let ttl: i64 = self.conn.ttl(key).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn hgetall(
        ...
        let map: std::collections::HashMap<String, String> =
            self.conn.hgetall(key).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn keys(&mut self, pattern: &str) -> Result<Vec<String>> {
        ...
        let keys: Vec<String> = self.conn.keys(pattern).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn incr(&mut self, key: &str) -> Result<i64> {
        ...
        let value: i64 = self.conn.incr(key, 1).await.map_err(|e| Error::Redis(e))?;
    ...
    pub async fn decr(&mut self, key: &str) -> Result<i64> {
        ...
        let value: i64 = self.conn.decr(key, 1).await.map_err(|e| Error::Redis(e))?;
