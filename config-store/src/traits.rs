...
use crate::types::{ConfigValue, ConfigError, ConfigTree, ConfigNode, ConfigVersion};
...
impl<'a> ConfigTransaction<'a> {
    ...
    pub fn set(&mut self, path: String, value: ConfigValue) {
        self.operations.push(TransactionOperation::Set { path, value });
    }
    ...
    pub fn set_node(&mut self, path: String, node: ConfigNode) {
        self.operations.push(TransactionOperation::SetNode { path, node });
    }
