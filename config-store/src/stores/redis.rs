...
impl RedisConfigStore {
    ...
    pub async fn new(redis_url: String, environment: String) -> Result<Self, ConfigError> {
        let client = Client::open(redis_url.as_str())
            .map_err(|e| ConfigError::OperationFailed(format!("Failed to create Redis client: {}", e)))?;
        ...
        let connection = match client.get_multiplexed_tokio_connection().await {
            ...
            Err(e) => {
                ...
                eprintln!("Warning: Redis connection failed, running in fallback mode: {}", e);
                None
            }
        };
    ...
    pub async fn with_ttl(redis_url: String, environment: String, ttl: Duration) -> Result<Self, ConfigError> {
    ...
    pub async fn set_if_not_exists(&self, path: &str, value: ConfigValue) -> Result<bool, ConfigError> {
        // Always update cache
    ...
    /// Bulk get operations
    pub async fn bulk_get(&self, paths: &[String]) -> Result<HashMap<String, ConfigValue>, ConfigError> {
...
impl ConfigStore for RedisConfigStore {
    async fn get(&self, path: &str) -> Result<ConfigValue, ConfigError> {
        ...
        if self.fallback_mode {
            return Err(ConfigError::NotFound(format!("Configuration not found: {}", path)));
        }
        ...
        if let Some(ref mut conn) = *conn_guard {
            ...
            match result {
                ...
                Ok(None) => Err(ConfigError::NotFound(format!("Configuration not found: {}", path))),
                Err(e) => {
                    eprintln!("Redis GET failed: {}", e);
                    Err(ConfigError::NotFound(format!("Configuration not found: {}", path)))
                }
            }
        } else {
            Err(ConfigError::NotFound(format!("Configuration not found: {}", path)))
    ...
    async fn get_version(&self, _path: &str, _version: u32) -> Result<ConfigValue, ConfigError> {
        ...
        Err(ConfigError::Custom("Version history not supported in Redis store".to_string()))
    }
    
    async fn get_history(&self, _path: &str) -> Result<Vec<crate::types::ConfigVersion>, ConfigError> {
        // History not implemented for Redis store
        // Would require additional data structure to store history
        Err(ConfigError::Custom("History not supported in Redis store".to_string()))
    }
    
    async fn set_node(&self, path: &str, node: crate::types::ConfigNode) -> Result<(), ConfigError> {
