...
async fn test_thread_safety() {
    ...
    for i in 0..100 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            store_clone
                .set(&path, ConfigValue::Integer(i as i64))
                .await
                .unwrap();
...
async fn test_data_integrity() {
    ...
    store
        .set("/complex/data", complex_value.clone())
        .await
        .unwrap();
    ...
    for i in 1..=15 {
        store
            .set("/versioned/data", ConfigValue::Integer(i))
            .await
            .unwrap();
    }
...
async fn test_hierarchical_operations() {
    ...
    store
        ...
        .unwrap();
    store
        .set("/app/cache/host", ConfigValue::String("redis".to_string()))
        .await
        .unwrap();
    store
        .set("/app/cache/port", ConfigValue::Integer(6379))
        .await
        .unwrap();
...
async fn test_production_scenarios() {
    ...
    store
        .set("/system/global/trading_hours", trading_hours.clone())
        .await
        .unwrap();
    ...
    store
        .set(
            "/system/feature_flags/enable_ml",
    ...
    store
        .set(
            "/system/feature_flags/enable_backtesting",
    ...
    if let Some(metadata) = retrieved_node.metadata {
        assert_eq!(
            ...
            Some("Base service config".to_string())
        );
    }
}
