...
async fn test_blocks_password_storage() {
    ...
    let result = store
        .set("/password", ConfigValue::String("mysecret".to_string()))
        .await;
...
async fn test_blocks_api_key_storage() {
    ...
    let result = store
        .set(
            "/stripe_api_key",
            ConfigValue::String("sk_live_123456".to_string()),
        )
        .await;
    assert!(result.is_err());
}
...
async fn test_validates_path_format() {
    ...
    let result = store
        .set("no_leading_slash", ConfigValue::String("value".to_string()))
        .await;
    assert!(result.is_err());

    let result = store
        .set("/../etc/passwd", ConfigValue::String("value".to_string()))
        .await;
    assert!(result.is_err());
}
...
async fn test_version_history() {
    ...
    // Set initial value
    store
        .set("/config/value", ConfigValue::Integer(1))
        .await
        .unwrap();

    // Update value multiple times
    store
        .set("/config/value", ConfigValue::Integer(2))
        .await
        .unwrap();
    store
        .set("/config/value", ConfigValue::Integer(3))
        .await
        .unwrap();
    ...
    assert!(
        history.len() >= 1,
        "History should have at least 1 version, got {}",
        history.len()
    );
...
async fn test_error_sanitization_in_production() {
    let store = SecureInMemoryConfigStore::new().with_production_mode();
