...
use neural_trader::{
    config::NeuralConfig,
    data::{MarketContext, TimeSeriesData},
    features::{
        market_microstructure::{MicrostructureAnalyzer, MicrostructureConfig},
        technical_indicators::{IndicatorConfig, TechnicalIndicatorEngine},
    },
...
fn bench_technical_indicators(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 5000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("compute_all", size), size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    let current = data.last().unwrap();
                    let historical = &data[..data.len() - 1];
                    let features = engine
                        .compute_all(black_box(current), black_box(historical))
                        .await
                        .unwrap();
                    features
                })
        ...
        group.bench_with_input(BenchmarkId::new("elliott_waves", size), size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    let current = data.last().unwrap();
                    let historical = &data[..data.len() - 1];
...
fn bench_microstructure_analysis(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("toxicity_metrics", size), size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    let current = data.last().unwrap();
                    let historical = &data[..data.len() - 1];
...
fn bench_neural_prediction(c: &mut Criterion) {
    ...
    for model in models {
        group.bench_function(&format!("predict_{}", model.to_lowercase()), |b| {
            ...
            b.iter(|| rt.block_on(async { predictor.predict(black_box(&context)).await.unwrap() }));
        });
    }
...
fn bench_end_to_end_pipeline(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("complete_pipeline", size), size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    ...
                    let technical = indicator_engine
                        .compute_all(current, historical)
                        .await
                        .unwrap();
                    ...
                    // Microstructure
                    microstructure
                        .analyze(current, historical, &mut all_features)
                        .await
                        .unwrap();
...
fn bench_ensemble_overhead(c: &mut Criterion) {
    ...
    let single_config = NeuralConfig {
        models: vec![("LSTM".to_string(), Default::default())]
            .into_iter()
            .collect(),
    ...
    group.bench_function("single_model", |b| {
        b.iter(|| {
            rt.block_on(async { single_predictor.predict(black_box(&context)).await.unwrap() })
        });
    });
...
fn calculate_momentum(data: &[TimeSeriesData]) -> f64 {
    ...
    let recent_avg = data[data.len() - 10..].iter().map(|d| d.close).sum::<f64>() / 10.0;
    let older_avg = data[data.len() - 20..data.len() - 10]
        .iter()
        .map(|d| d.close)
        .sum::<f64>()
        / 10.0;
