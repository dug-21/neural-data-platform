...
use autonomous_platform::neural::{NeuralPredictor, NeuralPredictorTrait, PredictionResult};
use chrono::Utc;
...
fn benchmark_ensemble_prediction(c: &mut Criterion) {
    ...
    group.bench_function("3_models", |b| {
        b.iter(|| {
            rt.block_on(async {
                let _ = predictor
                    .predict_ensemble(black_box(&data), 5, black_box(&models), None)
                    .await;
            })
        });
    });
        ...
        group.bench_with_input(
            ...
            |b, &horizon| {
                b.iter(|| {
                    rt.block_on(async {
                        let _ = predictor
                            .predict(black_box(&data), black_box(horizon), None)
                            .await;
                    })
                });
            },
