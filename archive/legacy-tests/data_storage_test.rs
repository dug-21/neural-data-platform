use autonomous_platform::data::storage::{PredictionData, TimeSeriesData, TimescaleDBStorage};
...
async fn test_batch_operations() {
    ...
    assert_eq!(
        results.len(),
        100,
        "Expected 100 data points after batch insert"
    );
}
...
async fn test_query_latest_predictions() {
    ...
    let latest = storage
        .get_latest_prediction("BTC/USD", "lstm_v1")
        .await
        .unwrap();
