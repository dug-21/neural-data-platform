...
mod redis_config_store_tests {
    ...
    async fn test_list_keys_with_prefix() {
        ...
        // Set multiple values
        let _ = store
            .set("/test/list/one", ConfigValue::String("1".to_string()))
            .await;
        let _ = store
            .set("/test/list/two", ConfigValue::String("2".to_string()))
            .await;
        let _ = store
            .set("/other/path", ConfigValue::String("3".to_string()))
            .await;
    ...
    async fn test_get_tree_with_redis() {
        ...
        let _ = store
            ...
            .await;
        let _ = store
            .set("/app/config/db/port", ConfigValue::Integer(5432))
            .await;
        let _ = store
            .set("/app/config/cache/ttl", ConfigValue::Integer(300))
            .await;
    ...
    async fn test_atomic_operations() {
        ...
        let result1 = store
            .set_if_not_exists("/test/atomic", value1.clone())
            .await;
    ...
    async fn test_connection_pooling() {
        ...
        let _ = store1
            .set("/test/pool1", ConfigValue::String("1".to_string()))
            .await;
        let _ = store2
            .set("/test/pool2", ConfigValue::String("2".to_string()))
            .await;
