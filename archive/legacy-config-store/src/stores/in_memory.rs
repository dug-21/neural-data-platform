...
use crate::traits::{path_utils, ConfigStore};
use crate::types::{
    ConfigError, ConfigNode, ConfigSnapshot, ConfigTree, ConfigValue, ConfigVersion,
};
...
impl InMemoryConfigStore {
    ...
    fn store_version(&self, path: &str, node: &ConfigNode) -> Result<(), ConfigError> {
        ...
        let version_entry = ConfigVersion::new(node.version, node.value.clone());
            ...
            match current {
                ConfigValue::Object(obj) => {
                    current = obj.get(*part).ok_or_else(|| {
                        ConfigError::NotFound(format!(
                            "Key '{}' not found in nested structure",
                            part
                        ))
                    })?;
                }
                _ => {
                    return Err(ConfigError::NotFound(format!(
                        "Cannot navigate into non-object value at '{}'",
                        part
                    )));
                }
            }
...
impl ConfigStore for InMemoryConfigStore {
    ...
    async fn get_history(&self, path: &str) -> Result<Vec<ConfigVersion>, ConfigError> {
        ...
        if let Some(current_node) = data.get(path) {
            let current_version =
                ConfigVersion::new(current_node.version, current_node.value.clone());
...
mod tests {
    ...
    async fn test_versioning() {
        ...
        // Set initial version
        store
            .set("/test/version", ConfigValue::Integer(1))
            .await
            .unwrap();

        // Update to create version 2
        store
            .set("/test/version", ConfigValue::Integer(2))
            .await
            .unwrap();
    ...
    async fn test_tree_operations() {
        ...
        store
            .set("/system/global", ConfigValue::Integer(1))
            .await
            .unwrap();
        store
            .set("/system/local", ConfigValue::Integer(2))
            .await
            .unwrap();
        store
            .set("/domain/trading", ConfigValue::Integer(3))
            .await
            .unwrap();
