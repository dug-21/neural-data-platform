...
mod config_store_trait_tests {
    ...
    async fn test_set_with_validation_failure() {
        ...
        mock.expect_set()
            ...
            .times(1)
            .returning(|_, _| {
                Err(ConfigError::ValidationFailed(vec![
    ...
    async fn test_get_version() {
        ...
        assert_eq!(
            result.unwrap(),
            ConfigValue::String("version_3_value".to_string())
        );
    }
    ...
    async fn test_get_version_not_found() {
        ...
        mock.expect_get_version()
            ...
            .returning(|path, version| {
                Err(ConfigError::VersionNotFound(path.to_string(), version))
            });
    ...
    async fn test_transaction_rollback_on_error() {
        ...
        mock.expect_delete().with(eq("/tx/3")).times(0);
...
mod path_utils_tests {
    ...
    fn test_parent_path() {
        assert_eq!(
            parent_path("/app/database/host"),
            Some("/app/database".to_string())
        );
