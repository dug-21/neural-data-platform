    ...
    pub fn validate_key(&self, key: &str) -> Result<(), ConfigError> {
        ...
        if key.is_empty() || key.len() > self.max_key_length {
            return Err(ConfigError::ValidationFailed(vec![format!(
                "Key length must be between 1 and {} characters",
                self.max_key_length
            )]));
        }
        ...
        match value {
            ConfigValue::String(s) => {
                ...
                if s.len() > self.max_value_size {
                    return Err(ConfigError::ValidationFailed(vec![format!(
                        "String value exceeds maximum size of {} bytes",
                        self.max_value_size
                    )]));
                }
            ...
            ConfigValue::Object(map) => {
                ...
                if map.len() > self.max_object_keys {
                    return Err(ConfigError::ValidationFailed(vec![format!(
                        "Object has too many keys ({}), maximum is {}",
                        map.len(),
            ...
            ConfigValue::Array(arr) => {
                ...
                if arr.len() > self.max_array_size {
                    return Err(ConfigError::ValidationFailed(vec![format!(
                        "Array too large ({}), maximum is {} items",
                        arr.len(),
                        self.max_array_size
                    )]));
                }
