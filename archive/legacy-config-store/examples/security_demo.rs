...
async fn main() {
    ...
    sneaky_config.insert(
        "password".to_string(),
        ConfigValue::String("hidden_secret".to_string()),
    );

    print!("Trying to store object with hidden password field ... ");
    match store
        .set("/database", ConfigValue::Object(sneaky_config))
        .await
    {
        Ok(_) => println!("❌ FAILED - Should have been blocked!"),
        Err(e) => println!("✅ BLOCKED"),
    ...
    for path in path_attacks {
        ...
        match store
            .set(path, ConfigValue::String("malicious".to_string()))
            .await
        {
            Ok(_) => println!("❌ FAILED - Should have been blocked!"),
            Err(e) => println!("✅ BLOCKED"),
    ...
    for path in injection_attacks {
        ...
        match store
            .set(path, ConfigValue::String("value".to_string()))
            .await
        {
            Ok(_) => println!("❌ FAILED - Should have been blocked!"),
            Err(e) => println!("✅ BLOCKED"),
