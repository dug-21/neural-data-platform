...
use autonomous_platform::{
    ...
    neural::{fann_predictor::FannPredictor, NeuralPredictor, NeuralPredictorTrait},

    // Strategy components
    strategies::{
        neural_enhanced::NeuralEnhancedStrategy, Position, StrategyConfig, TradingStrategy,
    },
};
...
async fn main() -> Result<()> {
    ...
    println!(
        "✅ FANN neural networks initialized with {} models",
        neural_config.models.len()
    );
    ...
    println!(
        "✅ Market data converted to neuro-divergent format with {} rows",
        df.height()
    );
    ...
    for i in 0..10 {
        ...
        let predictions = fann_predictor
            .predict_ensemble(current_data, 5, &neural_config.models, None)
            .await?;

        if let Some(pred) = predictions.first() {
            println!(
                "    - Predicted price: ${:.2} (confidence: {:.2}%)",
                pred.value,
        ...
        let signal = strategy
            .generate_signal(&market_ctx, position.as_ref())
            .await?;
    ...
    println!(
        "Final position: {}",
        if position.is_some() { "Open" } else { "Closed" }
    );
