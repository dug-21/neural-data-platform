...
use neural_trader::adapters::{
    ...
    daa_service::{DAAServiceAdapter, DAAMessage, TradingAction},
...
async fn main() -> anyhow::Result<()> {
    ...
    for (i, pred) in predicted_series.iter().take(3).enumerate() {
        println!("  T+{}: ${:.2} at {}", i + 1, pred.close, pred.timestamp.format("%H:%M:%S"));
    }
    ...
    println!("Feedback sent for decision: {}", feedback.correlation_id.unwrap());
