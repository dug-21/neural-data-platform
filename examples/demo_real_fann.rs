...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n📊 Prediction Results:");
    println!("   Current price: ${:.2}", market_data.last().unwrap().close);
    println!("   Predictions for next 6 periods:");
    for (i, pred) in prediction.predictions.iter().enumerate() {
        println!("   Period {}: ${:.2}", i + 1, pred);
    }
    println!("   Confidence: {:.1}%", prediction.confidence.unwrap_or(0.0) * 100.0);
    ...
    println!("   Throughput: ~{:.0} predictions/sec", 1000.0 / latency.as_millis() as f64);
