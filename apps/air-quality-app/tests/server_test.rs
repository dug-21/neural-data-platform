...
mod tests {
    ...
    async fn test_missing_required_params() {
        ...
        assert!(has_error, "Should fail validation for missing required params");
    }
    ...
    async fn test_response_formatting() {
        ...
        if let Ok(response) = result {
            ...
            assert!(response.is_error.is_some(), "Response should indicate error status");
        }
    }
    ...
    async fn test_tool_descriptions() {
        ...
        for tool in tools {
            ...
            assert!(schema.is_object(), "Tool {} schema should be a JSON object", tool.name());
        }
    }
    ...
    async fn test_tool_schemas_have_required_fields() {
        ...
        assert!(required.is_some(), "Schema should have required fields array");
        assert!(
            required.unwrap().iter().any(|v| v.as_str() == Some("location_id")),
