...
async fn test_load_config_from_etcd() {
    ...
    let client = ConfigClient::with_prefix(&["http://localhost:2379"], "/air-quality")
        .await
        .expect("Failed to connect to etcd");

    // Set test values
    client
        .set("/server/host", &json!("127.0.0.1"))
        .await
        .unwrap();
    client.set("/server/port", &json!(9090)).await.unwrap();
    client
        .set("/mqtt/broker_url", &json!("test-broker"))
        .await
        .unwrap();
    client.set("/mqtt/port", &json!(1884)).await.unwrap();
    client
        .set("/storage/base_path", &json!("./test-data"))
        .await
        .unwrap();

    // Load config using our module
    let config = air_quality_app::load_from_etcd()
        .await
...
async fn test_watch_config_changes() {
    ...
    assert!(
        ...
        "Watch callback should have been triggered"
    );
