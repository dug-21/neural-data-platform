...
impl AlertService for DefaultAlerts {
    ...
    fn get_recommendations(
        ...
        if let Some(co2) = data.co2 {
            if co2 > 1000.0 {
                recs.push(format!(
                    "Open windows for 15 minutes to reduce CO2 from {} to <1000 ppm",
                    co2.round()
                ));
            }
        }
        ...
        if let Some(pm25) = data.pm25 {
            if pm25 > 12.0 {
                recs.push(format!(
                    "Consider air purifier - PM2.5 is {} µg/m³ (Moderate)",
                    pm25
                ));
            }
        }
...
impl McpServer {
    pub async fn new() -> Result<Self> {
        ...
        tools.push(Box::new(AirQualityRecommendationsTool::new(
            store.clone(),
            alerts.clone(),
        )));
