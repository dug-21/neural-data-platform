# NDP Agent Creation Guide

This guide defines how to create new agents for the Neural Data Platform that align with established patterns, maintain consistency across the team, and leverage the AgentDB pattern memory system effectively.
...
Every NDP agent MUST have these sections:

```markdown
# NDP {Role Name}

You are the {role description} for the Neural Data Platform. {1-2 sentences on primary responsibility}.
...
Use the `get-pattern` skill to retrieve {domain} patterns for NDP.
...
Every NDP agent MUST reference these four skills:
...
Creating effective NDP agents requires:
