...
if echo "$PROMPT_LOWER" | grep -qiE "$PLAN_KEYWORDS"; then
  ...
  echo "Protocol: .claude/protocols/planning-protocol.md"
  echo ""
  echo "MANDATORY steps:"
  echo "  1. /get-pattern — search AgentDB for relevant patterns"
  echo "  2. Initialize via MCP hive-mind_init (do NOT use claude-flow swarm init CLI)"
  echo "  3. Seed shared memory via MCP memory_store (namespace = feature-id)"
  ...
  echo "  6. Generate IMPLEMENTATION-BRIEF.md, ACCEPTANCE-MAP.md, LAUNCH-PROMPT.md"
  echo ""
  echo "PLANNING RULES:"
...
if echo "$PROMPT_LOWER" | grep -qiE "$IMPL_KEYWORDS"; then
  ...
  echo "Protocol: .claude/protocols/implementation-protocol.md"
  echo ""
  echo "MANDATORY steps:"
  echo "  1. /get-pattern — search AgentDB for relevant patterns"
  echo "  2. Initialize via MCP hive-mind_init (do NOT use claude-flow swarm init CLI)"
...
if echo "$PROMPT_LOWER" | grep -qiE "$SWARM_KEYWORDS"; then
  ...
  echo "the appropriate protocol in .claude/protocols/"
  echo "  Planning:       .claude/protocols/planning-protocol.md"
  echo "  Implementation: .claude/protocols/implementation-protocol.md"
  echo ""
