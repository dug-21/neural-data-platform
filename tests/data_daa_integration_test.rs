...
use autonomous_platform::integration::data_access::{DataAccessLayer, DataRequest, DataResponse, Timeframe, PriceMap};
...
async fn test_data_access_layer_creation() {
    ...
    assert!(data_access.is_ok(), "Failed to create DataAccessLayer: {:?}", data_access.err());
    
    let layer = data_access.unwrap();
    assert!(layer.health_check().await.unwrap(), "DataAccessLayer health check failed");
}
...
async fn test_agent_data_request_handling() {
    ...
    assert!(response.is_ok(), "Failed to handle agent data request: {:?}", response.err());
...
async fn test_market_data_retrieval() {
    ...
    let btc_data = data_access.get_market_data("BTC/USD", Timeframe::Hourly).await;
    assert!(btc_data.is_ok(), "Failed to get BTC market data: {:?}", btc_data.err());
    ...
    let eth_data = data_access.get_market_data("ETH/USD", Timeframe::Hourly).await;
    assert!(eth_data.is_ok(), "Failed to get ETH market data: {:?}", eth_data.err());
...
async fn test_latest_prices_lookup() {
    ...
    // Get latest prices
    let price_map = data_access.get_latest_prices(symbols.iter().map(|s| s.to_string()).collect()).await;
    assert!(price_map.is_ok(), "Failed to get latest prices: {:?}", price_map.err());
    ...
    for (symbol, price_info) in prices {
        assert!(price_info.price > 0.0, "Price should be positive for {}", symbol);
...
async fn test_concurrent_agent_access() {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert!(response.success, "Agent {} request failed: {}", i, response.error_message.unwrap_or_default());
...
async fn test_caching_performance() {
    ...
    let first_response = data_access.get_market_data("BTC/USD", Timeframe::Hourly).await.unwrap();
    let first_duration = start.elapsed();
    
    // Second request (should hit cache)
    let start = std::time::Instant::now();
    let second_response = data_access.get_market_data("BTC/USD", Timeframe::Hourly).await.unwrap();
...
async fn test_agent_request_validation() {
    ...
    let response = data_access.handle_agent_data_request(invalid_time_request).await;
    assert!(response.is_err() || !response.unwrap().success);
}
...
async fn test_data_aggregation_and_analysis() {
    ...
    let response = data_access.handle_agent_data_request(aggregation_request).await;
    assert!(response.is_ok(), "Failed to handle aggregation request: {:?}", response.err());
...
async fn test_error_handling_and_recovery() {
    ...
    let response = data_access.handle_agent_data_request(request).await;
    assert!(response.is_ok(), "Error handling failed: {:?}", response.err());
...
async fn test_agent_data_stream_subscription() {
    ...
    let response = data_access.handle_agent_data_request(subscription_request).await;
    assert!(response.is_ok(), "Failed to handle subscription request: {:?}", response.err());
...
async fn test_performance_metrics_collection() {
    ...
    assert!(metrics.is_ok(), "Failed to get performance metrics: {:?}", metrics.err());
