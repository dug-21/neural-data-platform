...
use autonomous_platform::data::{RedisCache, PredictionResult};
...
async fn test_redis_connection() {
    ...
    assert!(cache_result.is_ok(), "Failed to connect to Redis: {:?}", cache_result.err());
}
...
async fn test_set_and_get_operations() {
    ...
    assert_eq!(retrieved.unwrap(), test_data, "Retrieved value doesn't match");
...
async fn test_get_nonexistent_key() {
    ...
    assert!(get_result.is_ok(), "Get should not fail for non-existent key");
    assert!(get_result.unwrap().is_none(), "Should return None for non-existent key");
}
...
async fn test_ttl_expiration() {
    ...
    assert!(get_result.is_ok() && get_result.unwrap().is_some(), "Value should exist immediately");
    ...
    assert!(get_result_after.unwrap().is_none(), "Value should have expired");
}
...
async fn test_cache_invalidation() {
    ...
    // Set a value
    cache.set("invalidate_test_key", &test_data, None).await
        .expect("Failed to set value");
    
    // Verify it exists
    let get_result: Result<Option<TestData>> = cache.get("invalidate_test_key").await;
    assert!(get_result.is_ok() && get_result.unwrap().is_some(), "Value should exist");
    ...
    assert!(get_result_after.is_ok(), "Get should succeed");
    assert!(get_result_after.unwrap().is_none(), "Value should be invalidated");
}
...
async fn test_prediction_caching() {
    ...
    let set_result = cache.set_prediction("prediction:BTCUSD", &prediction, 60).await;
    assert!(set_result.is_ok(), "Failed to cache prediction: {:?}", set_result.err());
    
    // Retrieve the prediction
    let get_result = cache.get_prediction("prediction:BTCUSD").await;
    assert!(get_result.is_ok(), "Failed to get prediction: {:?}", get_result.err());
...
async fn test_multiple_concurrent_operations() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            cache_clone.set(&format!("concurrent_key_{}", i), &test_data, Some(10)).await
        });
        handles.push(handle);
    }
    ...
    for i in 0..5 {
        let get_result: Result<Option<TestData>> = cache.get(&format!("concurrent_key_{}", i)).await;
