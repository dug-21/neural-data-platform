...
async fn test_real_fann_network_creation() {
    ...
    println!("   - Confidence: {:.2}%", result.confidence.unwrap_or(0.0) * 100.0);
    
    // Verify predictions are not mock values
    assert!(result.predictions.len() == 6);
    assert!(result.predictions[0] != 0.01, "Not using mock DeepAR value!");
...
async fn test_real_vendor_models() {
    ...
    assert!(deepar_result[0] != 0.01, "DeepAR is using mock implementation!");
...
async fn test_model_training() {
    ...
    println!("   - Confidence: {:.2}%", pred_after.confidence.unwrap_or(0.0) * 100.0);
...
async fn test_ensemble_with_real_models() {
    ...
    let config = NeuralConfig {
        models: vec!["LSTM".to_string(), "GRU".to_string(), "Transformer".to_string()],
        ensemble_method: "weighted_average".to_string(),
        lookback_window: 20,
    ...
    println!("   - All unique values: {}", 
        result.predictions.windows(2).all(|w| (w[0] - w[1]).abs() > 0.0001)
    );
    
    // Verify ensemble produces varied predictions
    let unique_count = result.predictions.iter()
        .collect::<std::collections::HashSet<_>>()
        .len();
    
    assert!(unique_count > 1, "Ensemble should produce varied predictions!");
