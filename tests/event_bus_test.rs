...
use autonomous_platform::streaming::{
    EventBusIntegration, EventSerializer, EventRouter, 
    MarketEvent, NewsEvent, QualityEvent, SystemEvent
};
use autonomous_platform::integration::data_access::{DataAccessLayer, DataRequest, Timeframe};
use autonomous_platform::data::{DataPipeline, TimescaleDBStorage, RedisCache};
...
async fn test_quality_event_publishing() -> Result<()> {
    ...
    event_bus.publish_quality_event(quality_event.clone()).await?;
...
async fn test_event_routing_to_daa_agents() -> Result<()> {
    ...
    event_bus.register_daa_agent("test_agent_001".to_string(), tx).await?;
    ...
    assert!(received_events.iter().any(|e| e.event_type == "market_event"));
...
async fn test_event_filtering_and_routing() -> Result<()> {
    ...
    // Publish events
    event_bus.publish_market_event(high_quality_event.clone()).await?;
    event_bus.publish_market_event(low_quality_event.clone()).await?;
...
async fn test_batch_event_processing() -> Result<()> {
    ...
    // Publish batch
    event_bus.batch_publish_market_events(market_events.clone()).await?;
...
async fn test_concurrent_event_publishing() -> Result<()> {
    ...
    let mut sequence_numbers: Vec<u64> = published_events
        .iter()
...
async fn test_memory_storage_integration() -> Result<()> {
    ...
    let event_summary = memory_data.get("event_summary").expect("Event summary should exist");
