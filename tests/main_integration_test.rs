...
use autonomous_platform::{
    integration::{
        daa_coordinator::{DaaCoordinator, DaaConfig, TradingAction},
...
async fn process_market_events_to_daa(
    ...
    for event in events {
        events_by_symbol.entry(event.symbol.clone())
        ...
        if time_series_data.len() >= 10 {
            ...
            if let Some(latest) = time_series_data.last() {
                ...
                match &decision.action {
                    TradingAction::Buy { symbol: sym, size, .. } => {
                        current_positions.insert(sym.clone(), Position {
                            symbol: sym.clone(),
                    ...
                    TradingAction::Sell { symbol: sym, size, .. } => {
                        if *size > 0.0 {
                            current_positions.insert(sym.clone(), Position {
                                symbol: sym.clone(),
...
mod main_flow_tests {
    ...
    async fn test_basic_event_flow() {
        ...
        for i in 0..15 {
            events.push(create_market_event("BTC/USDT", 50000.0 + (i as f64 * 10.0), i as u64));
        }
        
        let mut positions = HashMap::new();
        
        // Process events
        let decisions = process_market_events_to_daa(events, coordinator, &mut positions).await.unwrap();
    ...
    async fn test_multiple_symbols() {
        ...
        for (sym_idx, symbol) in symbols.iter().enumerate() {
            ...
            for i in 0..12 {
                events.push(create_market_event(symbol, base_price + (i as f64 * 5.0), i as u64));
            }
        }
    ...
    async fn test_position_management() {
        ...
        for i in 0..15 {
            events1.push(create_market_event("BTC/USDT", 50000.0 + (i as f64 * 50.0), i as u64));
        }
        
        let decisions1 = process_market_events_to_daa(events1, coordinator.clone(), &mut positions).await.unwrap();
        ...
        for i in 15..30 {
            events2.push(create_market_event("BTC/USDT", 51000.0 - (i as f64 * 30.0), i as u64));
        }
        
        let decisions2 = process_market_events_to_daa(events2, coordinator, &mut positions).await.unwrap();
    ...
    async fn test_volatility_calculation() {
        ...
        let mut positions = HashMap::new();
        let _ = process_market_events_to_daa(events, coordinator, &mut positions).await.unwrap();
        ...
        if let Ok(Some(decision)) = timeout(Duration::from_millis(100), rx.recv()).await {
            ...
            println!("Decision confidence with volatile market: {:.2}%", decision.confidence * 100.0);
    ...
    async fn test_concurrent_processing() {
        ...
        for symbol in symbols {
            ...
            let handle = tokio::spawn(async move {
                let mut events = Vec::new();
                for i in 0..15 {
                    events.push(create_market_event(&symbol, 1000.0 + (i as f64 * 10.0), i as u64));
                }
    ...
    async fn test_error_recovery() {
        ...
        // More valid events
        for i in 11..20 {
            events.push(create_market_event("BTC/USDT", 50100.0 + (i as f64 * 10.0), i as u64));
        }
    ...
    async fn test_high_frequency_events() {
        ...
        for batch in 0..5 {
            ...
            for i in 0..100 {
                ...
                events.push(create_market_event("BTC/USDT", 50000.0 + (seq as f64 * 10.0), seq));
            }
            all_events.extend(events);
        }
        
        let mut positions = HashMap::new();
        let decisions = process_market_events_to_daa(all_events, coordinator, &mut positions).await.unwrap();
