...
fn create_test_news_data(symbol: &str) -> NewsData {
    NewsData {
        ...
        content: format!("{} shows strong momentum in current market conditions", symbol),
    ...
    for i in 0..num_agents {
        ...
        let handle = tokio::spawn(async move {
            ...
            let decision_context = DecisionContext {
                ...
                market_data: create_time_series_data(&format!("CONCURRENT{}/USD", i), 1000.0 + (i as f64 * 100.0)),
                context_metadata: create_context_metadata(),
                required_confidence: 0.7,
                prediction_horizon: 60,
            };
            
            orchestrator_clone.get_neural_prediction(decision_context).await
        });
