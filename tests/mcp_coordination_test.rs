...
use autonomous_platform::data::{
    TimescaleDBStorage, RedisCache, DataPipeline, TimeSeriesData,
    PredictionResult, QualityMetrics, PlatformMetrics
};
use autonomous_platform::config::{
    PlatformConfig, DatabaseConfig, RedisConfig, NeuralConfig, 
    MonitoringConfig, PlatformInfo
};
...
fn create_test_config() -> PlatformConfig {
    PlatformConfig {
        ...
        neural: NeuralConfig {
            memory_gb: 1.0,
            models: vec!["NHITS".to_string(), "DeepAR".to_string(), "FANN".to_string()],
...
async fn test_end_to_end_data_flow() -> Result<()> {
    ...
    coordinator.data_pipeline.process_data(test_data.clone()).await?;
    ...
    let decision_response = coordinator.handle_external_request(decision_request).await?;
