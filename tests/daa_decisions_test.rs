...
use autonomous_platform::{
    integration::{
        ...
        TradeOrder, OrderSide, OrderType
    },
    data::TimeSeriesData,
    Result
};
...
async fn test_autonomous_decision_system_creation() {
    ...
    assert!(result.is_ok(), "Failed to create AutonomousDecisionSystem: {:?}", result.err());
    
    let system = result.unwrap();
    assert_eq!(system.get_agent_count(), 0, "New system should have no agents initially");
}
...
async fn test_spawn_trading_agents() {
    ...
    assert!(result.is_ok(), "Failed to spawn trading agents: {:?}", result.err());
    
    // Should have all 5 agent types
    assert_eq!(system.get_agent_count(), 5, "Should have spawned 5 trading agents");
    
    // Verify agent types are present
    assert!(system.has_agent(&AgentType::MarketAnalysis), "Should have MarketAnalysis agent");
    assert!(system.has_agent(&AgentType::RiskManagement), "Should have RiskManagement agent");
    assert!(system.has_agent(&AgentType::SignalGeneration), "Should have SignalGeneration agent");
    assert!(system.has_agent(&AgentType::Portfolio), "Should have Portfolio agent");
    assert!(system.has_agent(&AgentType::Execution), "Should have Execution agent");
}
...
async fn test_autonomous_decision_bullish_market() {
    ...
    assert!(decision.is_ok(), "Failed to make autonomous decision: {:?}", decision.err());
    ...
    match trading_decision.action {
        ...
        TradingAction::Sell => {
            ...
            assert!(trading_decision.risk_score > 0.7, "Sell in bullish market should have high risk justification");
        }
    }
}
...
async fn test_autonomous_decision_bearish_market() {
    ...
    match trading_decision.action {
        ...
        TradingAction::Buy => {
            ...
            assert!(trading_decision.confidence > 0.8, "Buy in bearish market should have very high confidence");
        }
    }
}
...
async fn test_multi_agent_consensus_decision() {
    ...
    assert!(consensus.is_ok(), "Failed to reach consensus decision: {:?}", consensus.err());
    
    let consensus_decision = consensus.unwrap();
    assert_eq!(consensus_decision.symbol, "AAPL");
    assert!(consensus_decision.agent_votes.len() >= 3, "Should have votes from multiple agents");
    assert!(consensus_decision.consensus_strength >= 0.0 && consensus_decision.consensus_strength <= 1.0);
    
    // Verify voting process
    let total_votes: f64 = consensus_decision.agent_votes.values().sum();
    assert!((total_votes - 1.0).abs() < 0.01, "Agent votes should sum to approximately 1.0");
}
...
async fn test_decision_engine_conflict_resolution() {
    ...
    assert!(decision.is_ok(), "Failed to resolve conflicting opinions: {:?}", decision.err());
    
    let trading_decision = decision.unwrap();
    
    // In conflicting scenarios, confidence should be moderate and action conservative
    assert!(trading_decision.confidence < 0.8, "High conflict should result in lower confidence");
    ...
    match trading_decision.action {
        ...
        TradingAction::Buy | TradingAction::Sell => {
            assert!(trading_decision.position_size < 0.5, "Conflicting signals should result in smaller position sizes");
        }
    }
}
...
async fn test_risk_management_agent() {
    ...
    assert!(decision.is_ok(), "Failed to handle high risk scenario: {:?}", decision.err());
    
    let trading_decision = decision.unwrap();
    
    // Risk management should override bullish signals in extreme risk scenarios
    assert!(trading_decision.risk_score > 0.7, "Should identify high risk");
    assert!(trading_decision.position_size < 0.3, "Should limit position size in high risk");
    
    // Should not recommend large buy positions despite bullish trend
    if let TradingAction::Buy = trading_decision.action {
        assert!(trading_decision.position_size < 0.2, "High risk should severely limit buy position size");
    }
}
...
async fn test_portfolio_agent_coordination() {
    ...
    let decision = system.make_autonomous_decision(market_context).await;
    assert!(decision.is_ok(), "Failed to make portfolio-aware decision: {:?}", decision.err());
    
    let trading_decision = decision.unwrap();
    
    // Portfolio agent should limit new positions when already well-allocated
    if let TradingAction::Buy = trading_decision.action {
        assert!(trading_decision.position_size <= 0.3, "Should not over-allocate when portfolio is balanced");
    }
}
...
async fn test_execution_agent_order_generation() {
    ...
    let decision = system.make_autonomous_decision(market_context).await;
    assert!(decision.is_ok(), "Failed to generate execution decision: {:?}", decision.err());
    ...
    if let TradingAction::Buy = trading_decision.action {
        ...
        assert!(order.is_ok(), "Failed to generate trade order: {:?}", order.err());
        ...
        match trade_order.order_type {
            ...
            OrderType::Limit => {
                ...
                assert!(limit_price > 140.0 && limit_price < 165.0, "Limit price should be within reasonable range");
            },
            _ => {
...
async fn test_autonomous_adaptation() {
    ...
    let initial_decision = system.make_autonomous_decision(initial_context).await.unwrap();
    ...
    let adapted_decision = system.make_autonomous_decision(changed_context).await.unwrap();
    
    // System should adapt its decision based on changed conditions
    assert_ne!(initial_decision.action, adapted_decision.action, 
               "Decision should change with market conditions");
    
    // Risk assessment should increase
    assert!(adapted_decision.risk_score > initial_decision.risk_score,
...
async fn test_system_performance_stress() {
    ...
    for symbol in symbols {
        ...
        let decision = system.make_autonomous_decision(market_context).await;
        assert!(decision.is_ok(), "Failed to make decision for {}: {:?}", symbol, decision.err());
    ...
    assert!(elapsed.as_millis() < 5000, "Decision making took too long: {:?}", elapsed);
    assert_eq!(decisions.len(), 5, "Should have made decisions for all symbols");
